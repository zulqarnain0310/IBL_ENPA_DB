...
CREATE PROCEDURE [dbo].[API_MAILALERT]--'IBL_ENPA_EXTRACTION STEP : Ganaseva_Provision_Stg_Daily' ,'Dear Ganaseva','Ganaseva ETL Staging Extraction was completed Successfully','Ganaseva ETL Staging Extraction was Failed',80,'RF'
...
SET @FEEDTYPE=@FEED
	...
	/****  Checksum CR changes ****/
		...
		--		+' Please find the forward feed details as process in the CRisMac on Today '+ ISNULL(@DS1_CS,'')+ISNULL(@DS2_CS,'')+ISNULL(@DS3_CS,'')+ISNULL(@DS4_CS,'')+ISNULL(@DS5_CS,'')+ISNULL(@DS6_CS,'')
				...
				Drop table if Exists #CheckSumFF_Table
					select FORMAT(ProcessDate,'dd-MM-yyyy') as Dateofdata,SourceName,DataSet,CRISMAC_CheckSum,Source_CheckSum,Start_BAU,Processing_Type,Reason,
					ApprovedBy into #CheckSumFF_Table
					from Dbo.CheckSumData_FF where SourceAlt_Key=@SourceAlt_Key AND EffectiveFromTimeKey=@Timekey And EffectiveToTimeKey=@Timekey


				DECLARE @xml_cs NVARCHAR(MAX)
				DECLARE @ChecksumTable NVARCHAR(MAX)

				SET @xml_cs = CAST(( SELECT 
				 [Dateofdata] AS 'td',''
				,[SourceName] AS 'td',''
				,[DataSet] AS 'td','' 
				,[CRISMAC_CheckSum] AS 'td',''
				,[Source_CheckSum] AS 'td',''
				,[Start_BAU] AS 'td','' 
				,[Processing_Type] AS 'td',''
				,ISNULL([Reason],'') AS 'td',''
				,ISNULL([ApprovedBy],'') AS 'td',''
				 FROM  #CheckSumFF_Table ORDER BY DataSet 
				FOR XML PATH('tr'), ELEMENTS ) AS NVARCHAR(MAX))

				SET @ChecksumTable ='<html><body><H3>Please find the forward feed details to be processed in the CRisMac on Today</H3>
				<table border = 1> 
				<tr>
				<th>Dateofdata</th><th>SourceName</th><th>DataSet</th><th>CRISMAC_CheckSum</th><th>Source_CheckSum</th><th>Start_BAU</th><th>Processing_Type</th><th>Reason</th><th>ApprovedBy'    

				SET @ChecksumTable = @ChecksumTable + @xml_cs +'</table></body></html>'
	...
	/****  Checksum CR changes ****/
		...
		--		Declare @CountString_RF Varchar(MAX)='Record Count for '+@SourceName+' is '+ISNULL(@Count,'')+' '+@RF_CS
			...
			Drop table if Exists #CheckSumRF_Table
				select FORMAT(AS_ON_DATE,'dd-MM-yyyy') as Dateofdata,SourceName,Convert(Varchar(20),RecordCount) as Count
				,CRISMAC_CheckSum as CRISMACRFCheckSum, Convert(Varchar(10),Datecreated,103)+' '+ Convert(Varchar(8),Datecreated,14) as DateTimestamp into #CheckSumRF_Table
				from Dbo.CheckSumData_RF where SourceAlt_Key=@SourceAlt_Key AND TimeKey=@Timekey 

				--select * from #CheckSumRF_Table

	
			DECLARE @RFxml_cs NVARCHAR(MAX)
			DECLARE @RFChecksumTable NVARCHAR(MAX)

			SET @RFxml_cs = CAST(( SELECT 
			 [Dateofdata] AS 'td',''
			,[SourceName] AS 'td',''
			,[Count] AS 'td','' 
			,[CRISMACRFCheckSum] AS 'td',''
			,[DateTimestamp] AS 'td' 
			FROM  #CheckSumRF_Table ORDER BY 1 
			FOR XML PATH('tr'), ELEMENTS ) AS NVARCHAR(MAX))

			SET @RFChecksumTable ='<html><body><H3>Please find the Reverse feed details from CRisMac for today</H3>
			<table border = 1> 
			<tr>
			<th>Dateofdata</th><th>SourceName</th><th>Count</th><th>CRISMACRFCheckSum</th><th>DateTimestamp'    

			SET @RFChecksumTable = @RFChecksumTable + @RFxml_cs +'</table></body></html>'

			--select @RFChecksumTable

			--select @RF_CS 
				Declare @CountString_RF Varchar(MAX)='Record Count for '+@SourceName+' is '+ISNULL(@Count,'')+' '+@RFChecksumTable
...
<Key>D2KUAT#20230622</Key>'+STRING_AGG(XML_COL,',')+'</Root>' AS BODY  FROM #BODY)
