...
GO
create PROCEDURE  [dbo].[SecurityErosion_bkp_21042023] 
(
@TimeKey int
)
AS
...
(SELECT A.NCIF_Id,MAX(NCIF_AssetClassAlt_Key) NCIF_AssetClassAlt_Key,SUM(Balance) Balance,SUM(PrincipleOutstanding)PrincipleOutstanding
FROM NPA_IntegrationDetails A With (nolock)
WHERE EffectiveFromTimeKey<=@Timekey
AND EffectiveToTimeKey>=@Timekey
AND SecuredFlag='Y'
