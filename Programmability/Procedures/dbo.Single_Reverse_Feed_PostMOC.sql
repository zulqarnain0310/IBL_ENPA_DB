...
CREATE PROC [dbo].[Single_Reverse_Feed_PostMOC](@TimeKey INT,@IS_MOC CHAR(1)='N') 
...
select * from ##Moc where AccountNo in (
										select AccountNo from ##Moc
										except
										select AccountNo from ##Current) 

Insert into #ReverseFeedDetails
select * from ##Current where AccountNo in (
											select AccountNo from ##Current
											except
											select AccountNo from ##Moc)
