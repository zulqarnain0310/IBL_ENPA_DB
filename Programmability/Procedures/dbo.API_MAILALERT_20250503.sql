...
CREATE PROCEDURE [dbo].[API_MAILALERT_20250503]--'IBL_ENPA_EXTRACTION STEP : Ganaseva_Provision_Stg_Daily' ,'Dear Ganaseva','Ganaseva ETL Staging Extraction was completed Successfully','Ganaseva ETL Staging Extraction was Failed',80,'RF'
	...
	/****  Checksum CR changes ****/
		...
		--		+' Please find the forward feed details as process in the CRisMac on Today '+ ISNULL(@DS1_CS,'')+ISNULL(@DS2_CS,'')+ISNULL(@DS3_CS,'')+ISNULL(@DS4_CS,'')+ISNULL(@DS5_CS,'')+ISNULL(@DS6_CS,'')
				...
				Drop table if Exists #CheckSumFF_Table
					...
					ApprovedByFirstLevel As FirstLevelApprovedBy,ApprovedBy AS SecondLevelApprovedBy into #CheckSumFF_Table
					from Dbo.CheckSumData_FF where SourceAlt_Key=@SourceAlt_Key AND EffectiveFromTimeKey=@Timekey And EffectiveToTimeKey=@Timekey
				...
				,ISNULL([Reason],'') AS 'td',''
				,ISNULL([FirstLevelApprovedBy],'') AS 'td',''
				,ISNULL([SecondLevelApprovedBy],'') AS 'td' 
				FROM  #CheckSumFF_Table ORDER BY DataSet 
				FOR XML PATH('tr'), ELEMENTS ) AS NVARCHAR(MAX))

				SET @ChecksumTable ='<html><body><H3>Please find the forward feed details as process in the CRisMac on Today</H3>
				<table border = 1> 
				<tr>
				<th>Dateofdata</th><th>SourceName</th><th>DataSet</th><th>CRISMAC_CheckSum</th><th>Source_CheckSum</th><th>Start_BAU</th><th>Processing_Type</th><th>Reason</th><th>FirstLevelApprovedBy</th><th>SecondLevelApprovedBy'    
