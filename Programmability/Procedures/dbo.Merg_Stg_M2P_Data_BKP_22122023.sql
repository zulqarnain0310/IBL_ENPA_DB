...
create PROC [dbo].[Merg_Stg_M2P_Data_BKP_22122023]
WITH RECOMPILE
AS
...
DECLARE @SrcSysAlt_Key Smallint=(SELECT SourceAlt_Key FROM [dbo].[DimSourceSystem] WHERE SourceName='M2P' AND  EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
...
DECLARE @Daily_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.M2P_Stg_Daily_Error)
DECLARE @Incremental_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.M2P_Stg_Incremental_Error)
--DECLARE @Security_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.CapitalFloat_Stg_Security_Error)
--DECLARE @Restructure_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.CapitalFloat_Stg_Restructure_Error)
...
SELECT @DataProcessingFlg=(CASE WHEN @Incremental_COUNT>0 OR @Daily_COUNT>0 --OR @Security_COUNT>0  OR @Restructure_COUNT>0
                                   OR  (SELECT ISNULL(COUNT(1),0) FROM IBL_ENPA_STGDB.DBO.M2P_Stg_Daily WHERE AsOnDate=@Exec_Date)=0
                                    THEN 0
                               ELSE 1
                           END)  
...
INSERT INTO [dbo].[LagDateDataDetails]
           ...
           ,[SourceSystem]
           ,[DataDateUsed])
SELECT @Exec_Date,'M2P',@DATEDATAUSE
END 

--Audit 
DELETE IBL_ENPA_STGDB.[dbo].[Procedure_Audit] 
WHERE [SP_Name]='Merg_Stg_M2P_Data' AND [EXT_DATE]=GETDATE() AND ISNULL([Audit_Flg],0)=0

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_Audit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Merg_Stg_M2P_Data',GETDATE()
...
INNER JOIN IBL_ENPA_STGDB.dbo.M2P_Stg_Incremental B ON A.EffectiveFromTimeKey<=@TimeKey
...
FROM NPA_IntegrationDetails A
INNER JOIN IBL_ENPA_STGDB.dbo.M2P_Stg_Daily B ON A.EffectiveFromTimeKey<=@TimeKey
                                                 AND A.EffectiveToTimeKey>=@TimeKey
												 AND B.AsONDATE=@Exec_Date
...
INNER JOIN IBL_ENPA_STGDB.dbo.M2P_Stg_Daily PSD ON NPA.SrcSysAlt_Key=@SrcSysAlt_Key
...
SELECT PSD.UCIF_ID, 
	   ...
	   'SSIS USER',
	   GETDATE()
FROM  IBL_ENPA_STGDB.dbo.M2P_Stg_Daily PSD 
...
FROM NPA_IntegrationDetails NPA
INNER JOIN IBL_ENPA_STGDB.dbo.M2P_Stg_Incremental PSD ON NPA.SrcSysAlt_Key=@SrcSysAlt_Key
...
SET [CustomerName]=FI.CustName,
    NCIF_Id=COALESCE(FI.UCIF_ID,FI.PAN,'CAP_'+FI.RefCustomerID),
    [PAN]=FI.PAN,
    [SanctionedLimit] =FI.SancLimit,
    [Segment] =FI.ActSegmentCode,
	[ProductCode]=FI.ProductCode,
	[ProductDesc] =FI.ProductName,
	[AC_AssetClassAlt_Key] =(CASE WHEN FI.SrcAssetClass in ('STD','SMA0','SMA1','SMA2')  THEN @STD
	                                              -- WHEN 103 THEN @WRE
	                                               WHEN FI.SrcAssetClass IN ('LOS','LOSS')  THEN @LOS   -- CURRENTLY NOT BEING PROVIDED
												   WHEN FI.SrcAssetClass='SUB'  THEN @SubSTD
												   WHEN FI.SrcAssetClass='DB1'  THEN @DB1      -- CURRENTLY NOT BEING PROVIDED
												   WHEN FI.SrcAssetClass='DB2'  THEN @DB2      -- CURRENTLY NOT BEING PROVIDED
												   WHEN FI.SrcAssetClass='DB3'  THEN @DB3      -- CURRENTLY NOT BEING PROVIDED
	                                               WHEN FI.SrcAssetClass='DBT' 
												                 THEN (CASE when DATEDIFF(DAY,SrcNpaDt,@Exec_Date)+1 between 366 and 730 then @DB1
							                                             ...
							                                             ELSE @DB1 --- DATA NEED TO BE CORRECTED 
	...
	[ProductType] =FI.FacilityType, ---- added by satish for reversefeed as on date 01 march 2023
...
INNER JOIN IBL_ENPA_STGDB.dbo.M2P_Stg_Incremental  FI ON EffectiveFromTimeKey<=@TimeKey
													    AND EffectiveToTimeKey>=@TimeKey
...
LEFT JOIN DimAssetClass DAC ON FI.SrcAssetClass=DAC.GanasevaAssetClassCode   --DAC.CapitalFloatAssetClassCode
...
--DECLARE @SrcSysAlt_Key Smallint=(SELECT SourceAlt_Key FROM [dbo].[DimSourceSystem] WHERE SourceName='CapitalFloat' AND  EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
...
LEFT JOIN IBL_ENPA_STGDB.dbo.CapitalFloat_Stg_Security SEC  ON  SEC.COLL_ID=ASD.CollateralID
...
-------Collateral_Delink_Details
INSERT INTO Collateral_Delink_Details(ASONDATE,SrcSysAlt_Key,NCIF_ID,CustomerId,CustomerACID,CollateralID,COLL_AMT,COLL_DELINK_DATE,COLL_TYPE)
SELECT SEC.AS_ON_DATE,@SrcSysAlt_Key,ENT_CIF,SRC_CIF,ACC_NO,COLL_ID,SEC.COLL_AMT,SEC.COLL_DELINK_DATE,SEC.COLL_TYPE
FROM IBL_ENPA_STGDB.dbo.CapitalFloat_Stg_Security SEC
INNER JOIN IBL_ENPA_DB_LOCAL_DEV.[CURDAT].AdvSecurityDetail ASD ON  ASD.EffectiveFromTimeKey<=@TimeKey
													  ...
													  AND  SEC.ACC_NO=ASD.RefSystemAcId
WHERE SEC.COLL_DELINK_Date IS NOT NULL


-----------SecurityValueDetail
DELETE FROM IBL_ENPA_DB_LOCAL_DEV.[CURDAT].AdvSecurityValueDetail  
Where EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey
AND SecurityEntityID IN (
SELECT ASD.SecurityEntityID
FROM IBL_ENPA_STGDB.dbo.CapitalFloat_Stg_Security SEC
INNER JOIN IBL_ENPA_DB_LOCAL_DEV.[CURDAT].AdvSecurityDetail ASD ON  ASD.EffectiveFromTimeKey<=@TimeKey
                                                      AND ASD.EffectiveToTimeKey>=@TimeKey
...
SELECT 0,COLL_TYPE,@SrcSysAlt_Key,ENT_CIF,SRC_CIF,ACC_NO,COLL_ID,@TimeKey,@TimeKey,'SSIS USER',GETDATE(),SEC.COLL_EXPRY_DATE
FROM IBL_ENPA_STGDB.dbo.CapitalFloat_Stg_Security SEC
...
GROUP BY COLL_TYPE,ENT_CIF,SRC_CIF,ACC_NO,COLL_ID,SEC.COLL_EXPRY_DATE
...
(SELECT COLL_ID,MIN(COLL_AMT) COLL_AMT
 FROM IBL_ENPA_STGDB.dbo.CapitalFloat_Stg_Security
 WHERE AS_ON_DATE=@EXT_DT
 GROUP BY AS_ON_DATE,/*ACC_NO,*/COLL_ID) SEC
 INNER JOIN IBL_ENPA_STGDB.dbo.CapitalFloat_Stg_Security STG ON SEC.COLL_ID=STG.COLL_ID
...
SET StkStmtDate=A.ValuationDate
   ,DPD_StockStmt=ISNULL(DATEDIFF(DAY,ValuationDate,@EXT_DT),0)+1
FROM 
...
IF((SELECT COUNT(DISTINCT AsOnDate) FROM IBL_ENPA_STGDB.dbo.M2P_Stg_Daily)=2
AND EXISTS(SELECT 1 FROM IBL_ENPA_STGDB.dbo.M2P_Stg_Daily WHERE ASonDate=@MonthEndDate)
...
FROM IBL_ENPA_STGDB.dbo.CapitalFloat_Stg_Restructure A
...
WHERE [SP_Name]='Merg_Stg_M2P_Data' AND [EXT_DATE]=@EXT_DT AND ISNULL([Audit_Flg],0)=0
