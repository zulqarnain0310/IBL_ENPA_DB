...
CREATE PROC [dbo].[AssetClassUpgradation_TEST_20240121](@TIMEKEY  INT)
WITH RECOMPILE
AS 
...
Drop table if exists #NPA_IntegrationDetails_TEST_20240119
Select  NCIF_Id
...
into #NPA_IntegrationDetails_TEST_20240119 from NPA_IntegrationDetails_TEST_20240119
where EffectiveFromTimeKey<=@TIMEKEY  and EffectiveToTimeKey>=@TIMEKEY

CREATE NONCLUSTERED INDEX idx_NPA_IntegrationDetail_temp ON #NPA_IntegrationDetails_TEST_20240119(CustomerACID)
...
--	                FROM #NPA_IntegrationDetails_TEST_20240119 NPA
...
SELECT DISTINCT	 A.NCIF_Id
	...
	AND Not Exists (Select 1 from #NPA_IntegrationDetails_TEST_20240119 NID 
...
/* COMMENTED RESSTR UPG CODE -- CODE COMMENTED ON 26092022 FOR RESTRICTING UPGRADATION WHILE NEW MODULE WAS UNDER DEVELOPMENT. FOR RESTRUCTNG NEW CODE ASS ADDED IN THE BEGINING OF SP */
 ...
 /* commented by satish as on 29092022 as from restructure it will be deleted down */
	...
	AND NOT Exists (SELECT 1 
	                FROM #NPA_IntegrationDetails_TEST_20240119 NID
...
--ON PROD 20240117
	...
	FROM NPA_IntegrationDetails_TEST_20240119 B WHERE B.NCIF_Id  IN (
														SELECT NCIFID_COBORROWER FROM COBORROWERDATA_CURNT  
																WHERE CustomerACID_PrimaryAccount IN (
																SELECT A.CUSTOMERACID	FROM NPA_IntegrationDetails_TEST_20240119 A
																INNER JOIN (SELECT NCIF_Id,SUM(CASE WHEN FlgUpg='Y' THEN 1 ELSE 0 END) COUNT
																		   FROM #NCIF_ASSET
																		   GROUP BY NCIF_Id) B ON A.EffectiveFromTimeKey<=@TIMEKEY
																								  AND A.EffectiveToTimeKey>=@TIMEKEY
...
select NCIF_ID,MAX(NCIF_AssetClassAlt_Key) NCIF_AssetClassAlt_Key,MIN(NCIF_NPA_Date) NCIF_NPA_Date,MIN(DateofImpacting) DateofImpacting 
	INTO #HOMOGINIZE_NCIF from NPA_IntegrationDetails_TEST_20240119 
...
UPDATE A SET A.NCIF_AssetClassAlt_Key=B.NCIF_AssetClassAlt_Key ,A.NCIF_NPA_Date=B.NCIF_NPA_Date,
  ...
  UPDATE NPA_IntegrationDetails_TEST_20240119 set FlgErosion=null
				,DbtDT =null
				,LossDT=nULL
				,ErosionDT=NULL  
...
Update A SET             
	...
	FROM [AdvAcRestructureCal] A            
		INNER JOIN NPA_IntegrationDetails_TEST_20240119 B ON A.CustomerAcid=B.CustomerAcId
...
DROP TABLE #NCIF_ASSET
DROP TABLE #Temp
DROP TABLE #NPA_IntegrationDetails_TEST_20240119
