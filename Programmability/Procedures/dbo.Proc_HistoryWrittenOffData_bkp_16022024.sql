...
Create Proc [dbo].[Proc_HistoryWrittenOffData_bkp_16022024]
(@Date Date)
As
Begin
Declare @Ext_Date Date=@Date
Declare @TimeKey_1 INT =(Select TimeKey From SysDataMatrix WHERE cast(date as date)=@Ext_Date)
Declare @TimeKey INT =@TimeKey_1-1
...
AND NCIF_Id IN (SELECT NCIF_ID FROM #npa_integrationdetails_curnt)

select 4
drop table if exists #AdvAcRestructureDetail_archive
Select * into #AdvAcRestructureDetail_archive from 
CURDAT.AdvAcRestructureDetail_archive
where  EffectiveFromTimeKey<=(Select @Timekey-1) and EffectiveToTimeKey>=(Select @Timekey-1)
AND RefCustomer_CIF IN (SELECT RefCustomer_CIF FROM #AdvAcRestructureDetail_curnt)
