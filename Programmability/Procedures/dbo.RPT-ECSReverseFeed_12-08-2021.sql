...
CREATE PROC [dbo].[RPT-ECSReverseFeed_12-08-2021]
	@TimeKey AS INT,
	@COST AS FLOAT
AS
...
AND MOC_AssetClassAlt_Key IS NOT NULL and SrcSysAlt_Key=20 and MOC_Status='Y' AND MOC_AssetClassAlt_Key<>1
ORDER BY NCIF_Id
...
,CASE WHEN PerculatedAccounts.ChangedAssetClass=1
	  THEN 'STD'
	  ELSE 'NPA'
...
,CASE WHEN PerculatedAccounts.ChangedAssetClass=1
	  THEN 'STD'
	  WHEN PerculatedAccounts.ChangedAssetClass=2
	  THEN 'SUB'
	  WHEN PerculatedAccounts.ChangedAssetClass=3
	  THEN 'DB1'
	  WHEN PerculatedAccounts.ChangedAssetClass=4
	  THEN 'DB2'
	  WHEN PerculatedAccounts.ChangedAssetClass=5
	  THEN 'DB3'
	  WHEN PerculatedAccounts.ChangedAssetClass=6
	  THEN 'LOS'
	  END																AS SourceSubClassification

,CONVERT(VARCHAR(25),PerculatedAccounts.ChangedNPADate,105)				AS SourceNPADate

,CASE WHEN ImpactedAccounts.ChangedAssetClass=1 
	  THEN 'STD'
	  ELSE 'NPA'
...
,CASE WHEN ImpactedAccounts.ChangedAssetClass=1
	  THEN 'STD'
	  WHEN ImpactedAccounts.ChangedAssetClass=2
	  THEN 'SUB'
	  WHEN ImpactedAccounts.ChangedAssetClass=3
	  THEN 'DB1'
	  WHEN ImpactedAccounts.ChangedAssetClass=4
	  THEN 'DB2'
	  WHEN ImpactedAccounts.ChangedAssetClass=5
	  THEN 'DB3'
	  WHEN ImpactedAccounts.ChangedAssetClass=6
	  THEN 'LOS'
...
WHERE PerculatedAccounts.SrcSysAlt_Key=20	
...
WHERE #MOCDATA.SrcSysAlt_Key=20	
AND PreProcessingFreeze='Y'
AND PreProcessingFreezeBy IS NOT NULL
AND PreProcessingFreezeDate IS NOT NULL
AND MOC_Freeze='Y'
