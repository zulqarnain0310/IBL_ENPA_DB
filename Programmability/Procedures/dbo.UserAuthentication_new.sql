...
--	@authType char(2)='AD'
		...
		--------------------------//Added By Shakti on 25102022  diss with Vinay sir
		declare @UserSusDay int =(select ParameterValue from DimUserParameters where EffectiveToTimeKey=49999 and ParameterType='Suspend Rights after Days of Non Use')
...
print 02
		...
		------- MODIFIED BY SATWAJI AS ON 08/02/2022 DUE TO REQUIREMENT CHANGE FROM AKASH SIR for DEACTIVATE FROM 45 DAYS TO 30 DAYS 
 
		-- Update A set A.Activate= Case When DATEDIFF(DD,LoginTime,GETDATE())>45 Then 'N' Else A.Activate End
		--Update A set A.Activate= Case When DATEDIFF(DD,LoginTime,GETDATE())>30 Then 'N' Else A.Activate End
		------- MODIFIED BY Shakti AS ON 25/10/2022 DUE TO REQUIREMENT CHANGE FROM Vinay Sir for DEACTIVATE FROM 30 DAYS TO DimuserParameter Parameter Date
		Update A set A.Activate= Case When DATEDIFF(DD,LoginTime,GETDATE())>@UserSusDay Then 'N' Else A.Activate End
		from DimUserInfo A
		INNER JOIN 
