...
CREATE PROCEDURE [dbo].[UserGroupInsertUpdate_13082021] -- -- select * from DimUserDeptGroup_Mod
 --@EntityKey int,        
 @DeptGroupId int = 1,        
...
print 'aaaaaaaa'						
							...
							IF(@AuthMode='N')
							(
								select @D2Ktimestamp=(SELECT  D2Ktimestamp FROM DimUserDeptGroup  WHERE  EffectiveFromTimeKey<=@TimeKey 
										AND DeptGroupCode = @DeptGroupName  AND ISNULL(AuthorisationStatus,'A')='A' )
							)
							RETURN @D2Ktimestamp
							RETURN @Result
