...
create PROCEDURE [dbo].[AuditAPISelectFeature_20250121]  
       @PANNo            VARCHAR(10) = N'',
		...
		If (@NCIF_Id_final<>'xxxx') and (@CountPAN=0)
				begin   
					...
					DROP  TABLE  IF exists #CoBo_Check
					   ...
					   into #CoBo_Check from  CoBorrowerData_curnt where NCIFID_COBORROWER in (@NCIF_Id_final,(select UCIC from #ucic))
				 
					IF EXISTS (select 1 from #CoBo_Check)

						--IF EXISTS (select 1 from  CoBorrowerData_curnt(nolock)
						--				where NCIFID_COBORROWER=@NCIF_Id_final OR  NCIFID_COBORROWER in (select UCIC from #ucic) ) 
							Begin
