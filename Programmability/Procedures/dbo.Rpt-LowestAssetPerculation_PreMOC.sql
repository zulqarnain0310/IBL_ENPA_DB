...
CREATE  proc [dbo].[Rpt-LowestAssetPerculation_PreMOC]
...
SELECT NCIF_Id,
		...
		MaxDPD,
		SrcSysAlt_Key
		,ROW_NUMBER() OVER (PARTITION BY NCIF_ID ORDER BY MAXDPD DESC)RANK  InTO #BILL
		FROM NPA_IntegrationBillDetails				
...
WHere NOT EXISTS(select CustomerACID from NPA_IntegrationBillDetails B WHErE a.CustomerACID=B.CustomerACID and B.EffectiveFromTimeKey<=@TimeKey ANd B.EffectiveToTimeKey>=@TimeKey)
AND A.EffectiveFromTimeKey<=@TimeKey AND A.EffectiveToTimeKey>=@TimeKey

OPTION(RECOMPILE)		 

CREATE INdEX I1 On #NPA_IntegrationDetails(NCIF_Id)

CREATE INdEX I2 On #NPA_IntegrationDetails(CustomerACID)
...
DISTINCT 
	   ...
	   ,NPA_IntegrationDetails.MOC_NPA_Date 
		...
		WHERE NPA_IntegrationDetails.AC_AssetClassAlt_Key=NCIF_AssetClassAlt_Key
		and NPA_IntegrationDetails.EffectiveFromTimeKey<=@TimeKey and NPA_IntegrationDetails.EffectiveToTimeKey>=@TimeKey
		and ISNULL(NPA_IntegrationDetails.AC_NPA_Date,'')=ISNULL(NCIF_NPA_Date,'')
		AND  ( NCIF_AssetClassAlt_Key IS NOT NULL AND NCIF_AssetClassAlt_Key<>1)
		...
		--AND NPA_IntegrationDetails.NCIF_Id='11485349'
...
NPA_IntegrationDetails.NCIF_Id,
	   ...
	   ,NPA_IntegrationDetails.MOC_NPA_Date 
		...
		INNER JOIN #BILL NPA_IntegrationBillDetails		ON  NPA_IntegrationDetails.EffectiveFromTimeKey<=@TimeKey and NPA_IntegrationDetails.EffectiveToTimeKey>=@TimeKey
														AND NPA_IntegrationBillDetails.NCIF_Id=NPA_IntegrationDetails.NCIF_Id	
		...
		AND NPA_IntegrationDetails.AC_AssetClassAlt_Key NOT IN (7)
		and ISNULL(NPA_IntegrationDetails.ProductAlt_Key,0)<>3200
		AND NCIF_AssetClassAlt_Key IS NOT NULL
		  AND ISNULL(NPA_IntegrationDetails.MOC_AssetClassAlt_Key,0)<>1
		--AND NPA_IntegrationDetails.NCIF_Id='11485349'
...
SELECT * INTO #TEMP1
FROM (select  DISTINCT NPA_IntegrationDetails.NCIF_Id,
		NPA_IntegrationDetails.ProductType,
		NPA_IntegrationDetails.CustomerName,
		NPA_IntegrationDetails.CustomerId,
		NPA_IntegrationDetails.PAN,
	   ...
	   ,NPA_IntegrationDetails.AstClsChngByUser
		...
		WHERE( (NPA_IntegrationDetails.AC_AssetClassAlt_Key<>NCIF_AssetClassAlt_Key  
		  OR ISNULL(NPA_IntegrationDetails.AC_NPA_Date,'')<>ISNULL(NCIF_NPA_Date,''))
		  and NPA_IntegrationDetails.EffectiveFromTimeKey<=@TimeKey and NPA_IntegrationDetails.EffectiveToTimeKey>=@TimeKey
...
select  DISTINCT NPA_IntegrationDetails.NCIF_Id,
	   ...
	   ,NPA_IntegrationDetails.AstClsChngByUser
		...
		INNER JOIN NPA_IntegrationBillDetails		ON  NPA_IntegrationDetails.EffectiveFromTimeKey<=@TimeKey and NPA_IntegrationDetails.EffectiveToTimeKey>=@TimeKey
													...
													AND NPA_IntegrationDetails.DPD_Overdrawn<>NPA_IntegrationBillDetails.MaxDPD
														--AND ISNULL(NPA_IntegrationDetails.Balance,0)<>0
...
,ImpactedAccounts.billno									AS IBillNo
...
,PerculatedAccounts.BILLNO									as PBILL
...
WHERE @Category=2												
AND (ImpactedAccounts.AstClsChngByUser='Y' 
     OR PerculatedAccounts.AstClsChngByUser='Y'
		  )
and PreProcessingFreeze='Y'
AND PreProcessingFreezeBy IS NOT NULL
AND PreProcessingFreezeDate IS NOT NULL
...
DROP TABLE #TEMP,#TEMP1,#BILL,#NPA_IntegrationDetails
