...
GO
CREATE Proc [dbo].[Dimproduct_Ganseva_Insert]
...
select  distinct  A.[SCHEME CODE  PRODUCT CODE],B.ProductCode,Dense_Rank() over(order by A.[SCHEME CODE  PRODUCT CODE]) newcode into #temp  from 
InduslndStg.dbo.Ganseva_Stg A
LEFT JOIN DIMPRODUCT B on A.[SCHEME CODE  PRODUCT CODE]=B.ProductCode
where B.ProductCode is null AND A.[SCHEME CODE  PRODUCT CODE]<>''
...
select @Product_Key+1, @ProductAlt_Key+10 ,[SCHEME CODE  PRODUCT CODE],@Timekey,49999  from #temp where newcode=@no
 set @no=@no+1
