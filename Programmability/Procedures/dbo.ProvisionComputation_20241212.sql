...
Create PROCEDURE [dbo].[ProvisionComputation_20241212] (@TimeKey Smallint,@IS_MOC CHAR(1)='N')
WITH RECOMPILE
AS
...
(CASE WHEN NID.NCIF_AssetClassAlt_Key=@SUB_Alt_Key AND (SELECT dbo.FullMonthsSeparation(@Ext_DATE_1, NID.NCIF_NPA_Date))<=3 THEN DPP.upto_3_months 
				...
				END) UNSEC_PER 
INTO ##TEMP FROM NPA_IntegrationDetails NID 
INNER JOIN DIMPROVISIONPOLICY DPP ON NID.ProductCode=DPP.Scheme_Code
and NID.EffectiveFromTimeKey<=@TimeKey and NID.EffectiveToTimeKey>=@TimeKey
...
--INSERT INTO ##TEMP
--SELECT NID.CustomerACID,NID.Provsecured,NID.ProvUnSecured,NID.SecuredAmt,NID.UnSecuredAmt,'N' PROVISION_COMP,
--DP.ProvisionSecured,DP.ProvisionUnSecured
--FROM NPA_IntegrationDetails NID INNER JOIN DimProvision DP ON NID.ProvisionAlt_Key=DP.ProvisionAlt_Key
--INNER JOIN HISTORY_PROVISIONPOLICY HDP ON -
--AND NID.EffectiveFromTimeKey<=@TIMEKEY AND NID.EffectiveToTimeKey>=@TIMEKEY
...
--SELECT CASE WHEN SEC_PROVPER_OLD>SEC_PER  THEN NID.SecuredAmt*NID.SEC_PROVPER_OLD
--ELSE NID.Provsecured END SEC ,
--CASE WHEN UNSEC_PROVPER_OLD>UNSEC_PER  THEN NID.UnSecuredAmt*NID.UNSEC_PROVPER_OLD
--ELSE NID.ProvUnsecured END 
--FROM NPA_IntegrationDetails NID INNER JOIN ##TEMP T ON NID.CustomerACID=T.CustomerACID
--AND NID.EffectiveFromTimeKey<=27092 AND NID.EffectiveToTimeKey>=27092
--AND ISNULL(NCIF_AssetClassAlt_Key,1)<>1
--WHERE T.PROVISION_COMP='Y'
--AND NID.CustomerACID IN (
--'5111577000281802','5114254000173898','5114254000187731','704000976339')


----SELECT 
----(CASE WHEN SEC_PROVPER_OLD>SEC_PER  THEN NID.SecuredAmt*NID.SEC_PROVPER_OLD ELSE NID.Provsecured END ),
----(CASE WHEN UNSEC_PROVPER_OLD>UNSEC_PER  THEN NID.UnSecuredAmt*NID.UNSEC_PROVPER_OLD ELSE NID.ProvUnsecured END )
----FROM NPA_IntegrationDetails NID INNER JOIN ##TEMP T ON NID.CustomerACID=T.CustomerACID
----AND ISNULL(NCIF_AssetClassAlt_Key,1)<>@STD_Alt_Key
----AND NID.EffectiveFromTimeKey<=@TIMEKEY AND NID.EffectiveToTimeKey>=@TIMEKEY
----WHERE T.PROVISION_COMP='Y'
----AND NID.CustomerACID IN (
----'5111577000281802','5114254000173898','5114254000187731','704000976339')
...
/*********** Bellow code commented and added by liyaqat on 20240625 for PROD observation  ************/

--UPDATE NID SET NID.Provsecured=(CASE WHEN SEC_PROVPER_OLD>SEC_PER  THEN NID.SecuredAmt*NID.SEC_PROVPER_OLD ELSE NID.Provsecured END ),
--NID.ProvUNsecured = (CASE WHEN UNSEC_PROVPER_OLD>UNSEC_PER  THEN NID.UnSecuredAmt*NID.UNSEC_PROVPER_OLD ELSE NID.ProvUnsecured END )
--FROM NPA_IntegrationDetails NID INNER JOIN ##TEMP T ON NID.CustomerACID=T.CustomerACID
--AND ISNULL(NCIF_AssetClassAlt_Key,1)<>@STD_Alt_Key
--AND NID.EffectiveFromTimeKey<=@TIMEKEY AND NID.EffectiveToTimeKey>=@TIMEKEY
--WHERE T.PROVISION_COMP='Y'

--UPDATE NID SET NID.SEC_PROVPER_OLD=(CASE WHEN SEC_PROVPER_OLD<SEC_PER  THEN SEC_PER   ELSE NID.SEC_PROVPER_OLD END ),
--NID.UNSEC_PROVPER_OLD = (CASE WHEN UNSEC_PROVPER_OLD<UNSEC_PER  THEN UNSEC_PER   ELSE NID.UNSEC_PROVPER_OLD END )
--FROM NPA_IntegrationDetails NID INNER JOIN ##TEMP T ON NID.CustomerACID=T.CustomerACID
--AND ISNULL(NCIF_AssetClassAlt_Key,1)<>@STD_Alt_Key
--AND NID.EffectiveFromTimeKey<=@TIMEKEY AND NID.EffectiveToTimeKey>=@TIMEKEY
	...
	UPDATE NID SET NID.SEC_PROVPER_OLD=(CASE WHEN ISNULL(SEC_PROVPER_OLD,0) < ISNULL(SEC_PER,0)  THEN SEC_PER   
				...
				NID.ProvisionAlt_Key = (CASE WHEN ISNULL(SEC_PROVPER_OLD,0) < ISNULL(SEC_PER,0)  THEN ProvisionAlt_Key   
												ELSE ISNULL(NID.ProvisionAlt_Key_Old,0) END) -- Added by Mayuresh on 20241114 for correct provision alt key tagging
				FROM NPA_IntegrationDetails NID INNER JOIN ##TEMP T ON NID.CustomerACID=T.CustomerACID
				AND ISNULL(NCIF_AssetClassAlt_Key,1)<>@STD_Alt_Key
				AND NID.EffectiveFromTimeKey<=@TIMEKEY AND NID.EffectiveToTimeKey>=@TIMEKEY

	UPDATE NID SET NID.ProvisionAlt_Key_Old = NID.ProvisionAlt_Key -- Added by Mayuresh on 20241114 for correct provision alt key tagging
				FROM NPA_IntegrationDetails NID INNER JOIN ##TEMP T ON NID.CustomerACID=T.CustomerACID
				AND ISNULL(NCIF_AssetClassAlt_Key,1)<>@STD_Alt_Key
				AND NID.EffectiveFromTimeKey<=@TIMEKEY AND NID.EffectiveToTimeKey>=@TIMEKEY
				AND ISNULL(SEC_PROVPER_OLD,0) < ISNULL(SEC_PER,0)



/***********************/

UPDATE NPA_IntegrationDetails SET TotalProvision=ISNULL(Provsecured,0)+ISNULL(ProvUnsecured,0) 
