...
CREATE PROC [dbo].[AssetClassUpgradation_BKP_20250509](@TIMEKEY  INT)
WITH RECOMPILE
AS 
...
Drop table if exists #NPA_IntegrationDetails_20250509
Select  NCIF_Id
...
into #NPA_IntegrationDetails_20250509 from NPA_IntegrationDetails_20250509
where EffectiveFromTimeKey<=@TIMEKEY  and EffectiveToTimeKey>=@TIMEKEY

CREATE NONCLUSTERED INDEX idx_NPA_IntegrationDetail_temp ON #NPA_IntegrationDetails_20250509(CustomerACID)
...
--SELECT NPA.NCIF_Id into #Restructured_NCI--	                FROM #NPA_IntegrationDetails_20250509 NPA
 ...
 /* START - 17062023 - RESTRUCTURE DEPLOYMENT */
  SELECT NPA.NCIF_Id  
	...
	into #Restructured_NCIF_Id
	                FROM #NPA_IntegrationDetails_20250509 NPA
...
SELECT DISTINCT	 A.NCIF_Id
	...
	AND Not Exists (Select 1 from #NPA_IntegrationDetails_20250509 NID 
...
/* COMMENTED RESSTR UPG CODE -- CODE COMMENTED ON 26092022 FOR RESTRICTING UPGRADATION WHILE NEW MODULE WAS UNDER DEVELOPMENT. FOR RESTRUCTNG NEW CODE ASS ADDED IN THE BEGINING OF SP */
 ...
 /* commented by satish as on 29092022 as from restructure it will be deleted down */
	...
	AND NOT Exists (SELECT 1 
	                FROM #NPA_IntegrationDetails_20250509 NID
...
/* START - CO-BORROWER UPG WORK*/--ON PROD 20240117
/*AMAR 11102023 - CHECK ACCOUNT WISE UPGRADE STSTUS FOR CO-BORROWER UPGRADE */
	UPDATE A
		SET  A.AcUpgFlg='Y'
			,A.AcUpgDate=@Processingdate
			,A.AcDegDate=NULL --ADDED BY ZAIN ON 20240205 BECAUSE OF OBSERVATION ON BOTH FLAGS CANNOT BE 'Y'
			,A.AcDegFlg=NULL --ADDED BY ZAIN ON 20240205 BECAUSE OF OBSERVATION ON BOTH FLAGS CANNOT BE 'Y'
	FROM CoBorrowerData_Curnt a
	INNER JOIN #NCIF_ASSET b
		ON A.CustomerACID_PrimaryAccount=b.CustomerACID
		WHERE B.FlgUpg='Y' and isnull(A.AcUpgFlg,'N')='N'

/*AMAR 11102023 - CO-BORROWER ENCIFID FOR UPGRADE */
	;WITH CTE_UPG_COBO
	AS
	(
		SELECT NCIFID_COBORROWER FROM CoBorrowerData_Curnt A
			WHERE ISNULL(A.AcUpgFlg,'N')='N' 
				AND ISNULL(A.AcDegFlg,'N')='Y'
			GROUP BY NCIFID_COBORROWER 
	)
	DELETE A
	FROM #NCIF_ASSET A
		INNER JOIN CTE_UPG_COBO B ON A.NCIF_Id=B.NCIFID_COBORROWER

	

	/* END - CO-BORROWER UPG WORK*/
...
--ON PROD 20240117
			/* UPDATING COBORROWER WHICH WAS DRAGGED BUT PRIMARY BORROWER UPGRADED HENCE COBO HAS TO BE UPGRADED*/--AS  PER OBSERVATION ADDEDBY ZAIN ON 20231223
	UPDATE B SET B.NCIF_AssetClassAlt_Key=@STD_Alt_Key,B.NCIF_NPA_Date=NULL,B.FlgUpg='Y',B.UpgDate=@Processingdate
				,B.NatureofClassification=NULL,B.DateofImpacting=NULL,B.ImpactingAccountNo=NULL,B.ImpactingSourceSystemName=NULL
	FROM NPA_IntegrationDetails_20250509 B WHERE B.NCIF_Id  IN (
														SELECT NCIFID_COBORROWER FROM COBORROWERDATA_CURNT  
																WHERE CustomerACID_PrimaryAccount IN (
																SELECT A.CUSTOMERACID	FROM NPA_IntegrationDetails_20250509 A
																INNER JOIN (SELECT NCIF_Id,SUM(CASE WHEN FlgUpg='Y' THEN 1 ELSE 0 END) COUNT
																		   FROM #NCIF_ASSET
																		   GROUP BY NCIF_Id) B ON A.EffectiveFromTimeKey<=@TIMEKEY
																								  AND A.EffectiveToTimeKey>=@TIMEKEY
														...
														AND B.NatureofClassification='D'
														AND B.EffectiveFromTimeKey=@TIMEKEY
														AND B.EffectiveToTimeKey=@TIMEKEY

/*HOMONOGIZING AFTER UPDATING DRAGGED TO STD FOR COBO IMPACT*/



/* INSERT DATA - UPGRADE RECORED IN COBORROWERDATA_HIST */
		...
		FROM [CoBorrowerData_Curnt] A
			INNER JOIN NPA_IntegrationDetails_20250509 B
				...
				AND (B.NCIF_AssetClassAlt_Key=1 AND AC_AssetClassAlt_Key >1
					)
			WHERE (AcDegFlg='Y' AND AcUpgFlg='Y') OR B.FlgUpg='Y'
		...
		FROM [CoBorrowerData_Curnt] A
			INNER JOIN NPA_IntegrationDetails_20250509 B
				...
				AND (B.NCIF_AssetClassAlt_Key=1 AND AC_AssetClassAlt_Key >1)
			WHERE (AcDegFlg='Y' AND AcUpgFlg='Y') OR B.FlgUpg='Y'
/*END - CO_BORROWER UPGRADE - */

/*HOMOGENIZING AFTER UPGRADING AS PER OBSERVATION OF 20231225*/

DROP TABLE IF EXISTS #HOMOGINIZE_NCIF
select NCIF_ID,MAX(NCIF_AssetClassAlt_Key) NCIF_AssetClassAlt_Key,MIN(NCIF_NPA_Date) NCIF_NPA_Date,MIN(DateofImpacting) DateofImpacting 
	INTO #HOMOGINIZE_NCIF from NPA_IntegrationDetails_20250509 
		WHERE NCIF_AssetClassAlt_Key NOT IN (1,7)
...
--SELECT A.NCIF_ID,A.NCIF_AssetClassAlt_Key,B.NCIF_AssetClassAlt_Key ,A.NCIF_NPA_Date,B.NCIF_NPA_Date  
--		from NPA_IntegrationDetails_20250509 A
--		INNER JOIN #HOMOGINIZE_NCIF B ON A.NCIF_ID=B.NCIF_Id
--		AND (A.NCIF_AssetClassAlt_Key IS NULL OR A.NCIF_NPA_Date IS NULL)

UPDATE A SET A.NCIF_AssetClassAlt_Key=B.NCIF_AssetClassAlt_Key ,A.NCIF_NPA_Date=B.NCIF_NPA_Date,
		A.NatureofClassification=(CASE WHEN A.NatureofClassification IS NULL  THEN 'V' ELSE A.NatureofClassification END)--NatureofClassification='V' /*COMMENTED DUE TO OBSERVATION OF NatureofClassification CHANGING WHILE HOMOGENIZING BY ZAIN ON 20240119*/
		,A.DateofImpacting=B.DateofImpacting
		 from NPA_IntegrationDetails_20250509 A
		INNER JOIN #HOMOGINIZE_NCIF B ON A.NCIF_ID=B.NCIF_Id
		AND (A.NCIF_AssetClassAlt_Key IS NULL OR A.NCIF_NPA_Date IS NULL OR A.DateofImpacting IS NULL)
		  and EffectiveFromTimeKey<=@TIMEKEY
			and EffectiveToTimeKey>=@TIMEKEY
			AND A.NCIF_AssetClassAlt_Key NOT IN (7)--ADDED AS PER OBSERVATION OF WRITE OFF CASES GETTING UPCLASS

UPDATE A SET A.NCIF_AssetClassAlt_Key=AC_AssetClassAlt_Key 
		 from NPA_IntegrationDetails_20250509 A
		 where EffectiveFromTimeKey<=@TIMEKEY
			and EffectiveToTimeKey>=@TIMEKEY
			AND A.AC_AssetClassAlt_Key IN (7)--ADDED AS PER OBSERVATION OF WRITE OFF CASES GETTING UPCLASS
  ...
  UPDATE NPA_IntegrationDetails_20250509 set FlgErosion=null
				,DbtDT =null
				,LossDT=nULL
				,ErosionDT=NULL  
				,NCIF_NPA_Date=NULL--ADDED DUE TO COBO OBSERVATION OF REVERSE FEED WITH NCIF_NPA_DATE OF STANDARD ACCOUNT BY ZAIN 20231218--ON PROD 20240117
  				,NatureofClassification=NULL --ADDED ON 20231109 BY ZAIN AS PER OBSERVATION WHILE TESTING FOR STANDARD CASES--ON PROD 20240117
				,ImpactingSourceSystemName=NULL --ADDED ON 20231109 BY ZAIN AS PER OBSERVATION WHILE TESTING FOR STANDARD CASES--ON PROD 20240117
				,ImpactingAccountNo=NULL --ADDED ON 20231109 BY ZAIN AS PER OBSERVATION WHILE TESTING FOR STANDARD CASES--ON PROD 20240117
				,DateofImpacting=NULL --ADDED ON 20231109 BY ZAIN AS PER OBSERVATION WHILE TESTING FOR STANDARD CASES--ON PROD 20240117
...
Update A SET             
	...
	FROM [AdvAcRestructureCal] A            
		INNER JOIN NPA_IntegrationDetails_20250509 B ON A.CustomerAcid=B.CustomerAcId
...
DROP TABLE #NCIF_ASSET
DROP TABLE #Temp
DROP TABLE #NPA_IntegrationDetails_20250509
