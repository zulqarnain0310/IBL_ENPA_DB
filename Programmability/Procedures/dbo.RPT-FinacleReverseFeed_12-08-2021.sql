...
------------===============Finacle Reverse Feed=======================================-------
/*
CREATED BY  : VEDIKA
CREATED DATE: 18-06-2018
DESCRIPTION : FINACLE REVERSE FEED
*/

CREATE PROC [dbo].[RPT-FinacleReverseFeed_12-08-2021]
	@TimeKey AS INT,
	@COST AS FLOAT
AS
...
AND MOC_AssetClassAlt_Key IS NOT NULL and SrcSysAlt_Key=10 and MOC_Status='Y' AND MOC_AssetClassAlt_Key<>1
ORDER BY NCIF_Id
...
,CASE WHEN PerculatedAsset.FinacleAssetClassCode='001' 
	  THEN 'STANDARD' 
	  ELSE 'NPA' 
...
,CASE WHEN PerculatedAsset.FinacleAssetClassCode ='001' 
	  THEN 'STD' 
	  WHEN PerculatedAsset.FinacleAssetClassCode ='002' 
	  THEN 'SUB'
	  WHEN PerculatedAsset.FinacleAssetClassCode ='003' 
	  THEN 'DB1' 
	  WHEN PerculatedAsset.FinacleAssetClassCode ='004' 
	  THEN 'DB2'
	  WHEN PerculatedAsset.FinacleAssetClassCode ='005' 
	  THEN 'DB3' 
	  WHEN PerculatedAsset.FinacleAssetClassCode ='006' 
	  THEN 'LOS'
	  END																AS SourceSubClassification

,CONVERT(VARCHAR(25),PerculatedAccounts.ChangedNPADate,105)				AS SourceNPADate

,CASE WHEN ImpactedAsset.FinacleAssetClassCode='001' 
	  THEN 'STANDARD' 
	  ELSE 'NPA' 
...
,CASE WHEN ImpactedAsset.FinacleAssetClassCode ='001' 
	  THEN 'STD' 
	  WHEN ImpactedAsset.FinacleAssetClassCode ='002' 
	  THEN 'SUB'
	  WHEN ImpactedAsset.FinacleAssetClassCode ='003' 
	  THEN 'DB1' 
	  WHEN ImpactedAsset.FinacleAssetClassCode ='004' 
	  THEN 'DB2'
	  WHEN ImpactedAsset.FinacleAssetClassCode ='005' 
	  THEN 'DB3' 
	  WHEN ImpactedAsset.FinacleAssetClassCode ='006' 
	  THEN 'LOS'
...
WHERE PerculatedAccounts.SrcSysAlt_Key=10	
...
----AND ImpactedAccounts.NCIF_Id='13155924 '
UNION ALL
...
,CASE WHEN MOCASSET.FinacleAssetClassCode='001' 
	  THEN 'STANDARD' 
	  ELSE 'NPA' 
	  END																AS CrossMappedMainClassification
...
,CASE WHEN MOCASSET.FinacleAssetClassCode ='001' 
	  THEN 'STD' 
	  WHEN MOCASSET.FinacleAssetClassCode ='002' 
	  THEN 'SUB'
	  WHEN MOCASSET.FinacleAssetClassCode ='003' 
	  THEN 'DB1' 
	  WHEN MOCASSET.FinacleAssetClassCode ='004' 
	  THEN 'DB2'
	  WHEN MOCASSET.FinacleAssetClassCode ='005' 
	  THEN 'DB3' 
	  WHEN MOCASSET.FinacleAssetClassCode ='006' 
	  THEN 'LOS'
	  END																AS CrossMappedSubClassification
...
WHERE #MOCDATA.SrcSysAlt_Key=10	
AND PreProcessingFreeze='Y'
AND PreProcessingFreezeBy IS NOT NULL
AND PreProcessingFreezeDate IS NOT NULL
AND MOC_Freeze='Y'
AND MOC_FreezeBy IS NOT NULL
AND MOC_FreezeDate IS NOT NULL
----AND #MOCDATA.NCIF_Id='13155924 '
) A  WHERE RANK=1

OPTION(RECOMPILE)

DROP TABLE #Impacted,#Percolated,#MOCDATA
