...
GO
CREATE PROCEDURE [dbo].[GetFreezeDateInfo_12082021]
...
ELSE
 ...
 --PRINT @NextMonthEndDate
  ...
  ISNULL(Finacle_Reverse,'N') as Finacle_Reverse,ISNULL(ECS_Reverse,'N') as ECS_Reverse,ISNULL(Prolendz_Reverse,'N') as Prolendz_Reverse, ISNULL(Ganseca_Reverse,'N') as Ganseva_Reverse ,ISNULL(Calypso_Reverse,'N') as Calypso_Reverse,ISNULL(ETL_Completed,'N') as ETL_Completed,CONVERT(VARCHAR(10), @NextDate,103) as NextDate,isnull(PNPA_Status,'N') as PNPA_Status,'TblFreezeDate' AS TableName FROM SysDatamatrix 
  WHERE  ExtDate=@ExtDate and CurrentStatus='C'
END
