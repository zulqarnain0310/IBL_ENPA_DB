...
CREATE Proc [dbo].[FF_CRISMACChecksumCreate]
...
Declare @Srcname Varchar (20) =(select  Case when @SourceAlt_Key=40		Then 'Calypso'
		...
		Insert into #CheckSumData_FF 
			...
			----Declare @Src Varchar(8)=(select UPPER(SUBSTRING(@Srcname,0,4))) -- Added by Liyaqat on 20241108
			Declare @Src Varchar(8)=(select Case When @Srcname='Finacle2' Then 'FI2' Else UPPER(SUBSTRING(@Srcname,0,4))End ) 
			Declare @date Varchar(8)=(select Replace((convert(Varchar,@Ext_Date,103)),'/',''))
Print 1
		If @DS1 is not null
			Begin
			Drop Table if exists TempDS1
					Exec ('select Count(1) as count, SUM(Balance) Total into TempDS1 from '+@DS1)
...
Print 2
		If @DS2 is not null
			...
			Drop Table if exists Count_Assetclass
					Exec ('select Count(1) as Total into Count_Assetclass from '+@DS2+' Where ISNULL(SrcAssetClass,'''')<>''''' ) 
