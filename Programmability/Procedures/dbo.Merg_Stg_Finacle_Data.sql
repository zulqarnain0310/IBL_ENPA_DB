...
CREATE PROC [dbo].[Merg_Stg_Finacle_Data]
WITH RECOMPILE
AS
...
DECLARE @SrcSysAlt_Key Smallint=(SELECT SourceAlt_Key FROM [dbo].[DimSourceSystem] WHERE SourceName='Finacle' AND  EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
...
DECLARE @PrevMonthEndDate DATE=(SELECT EOMONTH(DATEADD(MONTH,-1,@Exec_Date)))--ADDED ON uat 20230612 FOR STD PROVISION deployed on production 20230707
DECLARE @PrevMonthEndTimekey Smallint=(SELECT TimeKey FROM SysDataMatrix WHERE CAST(DATE AS DATE)=@PrevMonthEndDate)--ADDED ON uat 20230612 FOR STD PROVISION deployed on production 20230707
DECLARE @PrevMoc_Freeze varchar(5)=(SELECT MOC_Freeze FROM SysDataMatrix WHERE TimeKey=@PrevMonthEndTimekey)--ADDED ON uat 20230612 FOR STD PROVISION deployed on production 20230707

DECLARE @Daily_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Daily_Error)
DECLARE @Incremental_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Incremental_Error)
DECLARE @Security_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Security_Error)
DECLARE @Restructure_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Restructure_Error)
DECLARE @PROVISION_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Finacle_Provision_Stg_Daily_Error)--ADDED ON uat 20230612 deployed on production 20230707
...
SELECT @DataProcessingFlg=(CASE WHEN @Incremental_COUNT>0 OR @Daily_COUNT>0 OR @Security_COUNT>0  OR @Restructure_COUNT>0
                                   OR  (SELECT ISNULL(COUNT(1),0) FROM IBL_ENPA_STGDB.DBO.Finacle_Stg_Daily WHERE AsOnDate=@Exec_Date)=0
                                    THEN 0
                               ELSE 1
                           END)  
...
INSERT INTO [dbo].[LagDateDataDetails]
           ...
           ,[SourceSystem]
           ,[DataDateUsed])
SELECT @Exec_Date,'Finacle',@DATEDATAUSE
END
--Audit 
DELETE IBL_ENPA_STGDB.[dbo].[Procedure_Audit] 
WHERE [SP_Name]='Merg_Stg_Finacle_Data' AND [EXT_DATE]=GETDATE() AND ISNULL([Audit_Flg],0)=0

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_Audit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Merg_Stg_Finacle_Data',GETDATE()
...
--------------For Sub Audit Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_NCIF_Update',GETDATE()
...
INNER JOIN IBL_ENPA_STGDB.dbo.Finacle_Stg_Incremental B ON A.EffectiveFromTimeKey<=@TimeKey
...
FROM NPA_IntegrationDetails A
INNER JOIN IBL_ENPA_STGDB.dbo.Finacle_Stg_Daily B ON A.EffectiveFromTimeKey<=@TimeKey
                                                 AND A.EffectiveToTimeKey>=@TimeKey
												 AND B.AsONDATE=@Exec_Date
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_NCIF_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit Delete Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_Delete_ClosedAccounts',GETDATE()
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_Delete_ClosedAccounts' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0


--------------For Sub Audit Daily Existing Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_DailyExisting_Update',GETDATE()
...
INNER JOIN IBL_ENPA_STGDB.dbo.Finacle_Stg_Daily PSD ON NPA.SrcSysAlt_Key=@SrcSysAlt_Key
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_DailyExisting_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0


--------------For Sub Audit Insert New Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_InsertNewAccounts',GETDATE()
...
SELECT PSD.UCIF_ID, 
	   ...
	   'SSIS USER',
	   GETDATE()
FROM  IBL_ENPA_STGDB.dbo.Finacle_Stg_Daily PSD 
...
FROM NPA_IntegrationDetails NPA
INNER JOIN IBL_ENPA_STGDB.dbo.Finacle_Stg_Incremental PSD ON NPA.SrcSysAlt_Key=@SrcSysAlt_Key
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_InsertNewAccounts' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit Incremental Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_Incremental_Update',GETDATE()

--Updating data From incremental table
UPDATE NI
SET [CustomerName]=FI.CustName,
    NCIF_Id=COALESCE(FI.UCIF_ID,FI.PAN,'FNCL_'+FI.RefCustomerID),
    [PAN]=FI.PAN,
	...
	,NI.NatureofClassification=(CASE WHEN FI.NATUREOFCLASSIFICATION='' THEN NULL --ON PROD 20240117
									WHEN FI.NATUREOFCLASSIFICATION IS NULL THEN NULL 
									ELSE FI.NATUREOFCLASSIFICATION END)--20231026 FOR COBORROWER BY ZAIN
FROM NPA_IntegrationDetails NI
INNER JOIN IBL_ENPA_STGDB_LOCAL_DEV.dbo.Finacle_Stg_Incremental  FI ON EffectiveFromTimeKey<=@TimeKey
													    AND EffectiveToTimeKey>=@TimeKey
													    ...
													    AND CONVERT(DATE,FI.AsOnDate,103)=@Exec_Date
LEFT JOIN DimAssetClass DAC ON FI.SrcAssetClass=DAC.FinacleAssetClassCode 
                           AND DAC.EffectiveFromTimeKey<=@TimeKey
						   AND DAC.EffectiveToTimeKey>=@TimeKey


--20231026 FOR COBORROWER BY ZAIN --ON PROD 20240117
UPDATE NI
  SET NI.DateofImpacting=ISNULL(CAST(FI.DateofImpacting AS DATE),AC_NPA_Date)
FROM NPA_IntegrationDetails NI
INNER JOIN IBL_ENPA_STGDB_LOCAL_DEV_LOCAL_DEV.dbo.Finacle_Stg_Incremental  FI ON EffectiveFromTimeKey<=@TimeKey
													    AND EffectiveToTimeKey>=@TimeKey
                                                        AND ISNULL(NI.NCIF_Id,'')=ISNULL(FI.UCIF_ID,'')
												        AND NI.CustomerId=FI.RefCustomerID
												        AND NI.CustomerACID=FI.CustomerAcID
  ...
  WHERE NI.NATUREOFCLASSIFICATION IS NOT NULL


  
/* Restrict DPD calculation basis renewal date for TL/DL cases of Finacle  done at 22092022 as   per change request by bank */

 Update A set DPD_Renewals=Null from  NPA_IntegrationDetails A
  where  EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey
  AND SrcSysAlt_Key=@SrcSysAlt_Key   
  and isnull(FacilityType,'')='LAA'
					
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_Incremental_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0


--------------For Sub Audit Write Off update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_WO_Balance_Update',GETDATE()
...
--------------- Changes done for Sale to ARC balance and principalouttanding need to make as zero---------
UPDATE NPA_IntegrationDetails 
SET PrincipleOutstanding=0,UNSERVED_INTEREST=0 ,Balance=0,IntOverdue=0    ------ Added on 16-06-2021 for Balance,IntOverdue by sunil
WHERE EffectiveFromTimeKey<=@TimeKey
AND EffectiveToTimeKey>=@TimeKey
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_WO_Balance_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit Renewal update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_Renewal_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_Renewal_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit WO Columns update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_WO_Columns_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_AdvAcWODetail_Insert' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
/*
UPDATE IBL_ENPA_DB.[CURDAT].AdvSecurityDetail
...
UPDATE IBL_ENPA_DB.[CURDAT].AdvSecurityValueDetail
SET EffectiveFromTimeKey=@TimeKey,
    EffectiveToTimeKey=@TimeKey
FROM IBL_ENPA_DB.[CURDAT].AdvSecurityValueDetail  A
INNER JOIN IBL_ENPA_DB.[CURDAT].AdvSecurityDetail B ON   
...
INTO #ENTITYID 
FROM IBL_ENPA_DB.dbo.AdvSecurityDetail ASD 
LEFT JOIN IBL_ENPA_STGDB.dbo.Finacle_Stg_Security SEC  ON  SEC.COLL_ID=ASD.CollateralID
...
--------------For Sub Audit AdvSecurityDetail Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_AdvSecurityDetail_Insert',GETDATE()

INSERT INTO IBL_ENPA_DB.[CURDAT].AdvSecurityDetail
...
FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Security SEC
...
FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Security SEC
INNER JOIN IBL_ENPA_DB.[CURDAT].AdvSecurityDetail ASD ON  ASD.EffectiveFromTimeKey<=@TimeKey
                                                      AND ASD.EffectiveToTimeKey>=@TimeKey
													  AND SEC.AS_ON_DATE=@EXT_DT
													  AND ASD.SrcSystemAlt_Key=@SrcSysAlt_Key
                                                      AND SEC.COLL_ID=ASD.CollateralID
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_AdvSecurityDetail_Insert' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit SecurityEntityID Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_SecurityEntityID_Update',GETDATE()
...
FROM 
(SELECT CollateralID 
FROM  IBL_ENPA_DB.[CURDAT].AdvSecurityDetail
WHERE EffectiveFromTimeKey<=@TimeKey
AND EffectiveToTimeKey>=@TimeKey
AND SrcSystemAlt_Key=@SrcSysAlt_Key
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_SecurityEntityID_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit Security Value Columns Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_SecurityValueColumns_Update',GETDATE()
...
INNER JOIN  IBL_ENPA_DB.[CURDAT].AdvSecurityDetail ASD ON ASD.EffectiveFromTimeKey<=@TimeKey
													   ...
													   AND SEC.ACC_NO=ASD.RefSystemAcId 
													   AND ASD.SrcSystemAlt_Key=@SrcSysAlt_Key
INNER JOIN  IBL_ENPA_DB.[CURDAT].AdvSecurityValueDetail ASVD  ON ASVD.EffectiveFromTimeKey<=@TimeKey
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_SecurityValueColumns_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit AdvSecurityValueDetail Insert Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_AdvSecurityValueDetail_Insert',GETDATE()

--INSERT NEW RECORDS IN AdvSecurityValueDetail(Populating Min coll value if same coll_id appering more than one)
INSERT INTO IBL_ENPA_DB.[CURDAT].AdvSecurityValueDetail(
...
(SELECT COLL_ID,MIN(COLL_AMT) COLL_AMT
 FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Security
 WHERE AS_ON_DATE=@EXT_DT
 GROUP BY AS_ON_DATE/*,ACC_NO*/,COLL_ID) SEC
 INNER JOIN IBL_ENPA_STGDB.dbo.Finacle_Stg_Security STG ON SEC.COLL_ID=STG.COLL_ID
...
FROM IBL_ENPA_DB.[CURDAT].AdvSecurityDetail ASD 
LEFT JOIN  IBL_ENPA_DB.[CURDAT].AdvSecurityValueDetail ASVD  ON  ASVD.EffectiveFromTimeKey<=@TimeKey
                                                               AND ASVD.EffectiveToTimeKey>=@TimeKey
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_AdvSecurityValueDetail_Insert' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0


--------------For Sub Audit stock Statement Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_StkStmtDate_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_StkStmtDate_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit MaxDPD Update Added on 18-07-2021 ----------------
/*    Commented on 12/08/2021
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_MaxDPD_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_MaxDPD_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
--------------For Sub Audit Security Prev Value Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_SecurityPrevValue_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_SecurityPrevValue_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
IF((SELECT COUNT(DISTINCT AsOnDate) FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Daily)=2
AND EXISTS(SELECT 1 FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Daily WHERE ASonDate=@MonthEndDate)
AND (SELECT ISNULL(MOC_Initialised,'N') FROM SysDataMatrix WHERE Date=@MonthEndDate)='N')
BEGIN

--------------For Sub Audit NPA Month End Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_NPAMonthEnd_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_NPAMonthEnd_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
--------------For Sub Audit AdvAcRestructureDetail Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_AdvAcRestructureDetail_Update',GETDATE()
...
FROM IBL_ENPA_STGDB.dbo.Finacle_Stg_Restructure A
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_AdvAcRestructureDetail_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit AdvAcRestructureDetail Insert Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Finacle_AdvAcRestructureDetail_Insert',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Finacle_AdvAcRestructureDetail_Insert' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

END


--STD PROVISION added on UAT 20230612 DEPLOYED ON PROD 20230707
...
SELECT @Exec_Date as Date_of_Change,getdate() as DateCreated,A.NCIF_Id,A.CustomerId,A.CustomerACID,A.Balance,A.STD_ASSET_CAT_Alt_key,B.STD_ASSET_CATAlt_key,@PrevMoc_Freeze  MOC_status,'Finacle'
FROM NPA_IntegrationDetails A
INNER JOIN IBL_ENPA_STGDB.dbo.Finacle_Provision_Stg_Daily P ON  A.NCIF_Id=P.ENT_CIF
												 AND A.CustomerID=P.SRC_CIF
												 AND A.CustomerACID=P.ACC_NO	
												 AND A.EffectiveFromTimeKey<=@TIMEKEY
												 AND A.EffectiveToTimeKey>=@TIMEKEY
												 AND CAST( P.AS_ON_DATE AS date) = @Exec_Date
INNER JOIN IBL_ENPA_DB.dbo.DIM_STD_ASSET_CAT B ON  B.EffectiveFromTimeKey<=@TIMEKEY
                                                 AND B.EffectiveToTimeKey>=@TIMEKEY
												 AND P.STD_ASSET_CAT=B.STD_ASSET_CATShortNameEnum
												WHERE SrcSysAlt_Key IN (@SrcSysAlt_Key,50)--ADDED ON 20231109 INCLUDING FOR ECBF ONPROD 20240117
...
INNER JOIN IBL_ENPA_DB.dbo.DIM_STD_ASSET_CAT B ON  B.EffectiveFromTimeKey<=@TIMEKEY
                                                 AND B.EffectiveToTimeKey>=@TIMEKEY
												 AND P.STD_ASSET_CAT=B.STD_ASSET_CATShortNameEnum
												WHERE SrcSysAlt_Key IN (@SrcSysAlt_Key)--ADDED ON 20231109 INCLUDING FOR ECBF --ON PROD 20240117
...
UPDATE IBL_ENPA_DB.DBO.NPA_IntegrationDetails SET STD_ASSET_CAT_Alt_key=900 
WHERE EffectiveFromTimeKey=@TIMEKEY AND 
SrcSysAlt_Key IN (@SrcSysAlt_Key,50)--ADDED ON 20231109 INCLUDING FOR ECBF --ON PROD 20240117
...
INNER JOIN IBL_ENPA_STGDB.dbo.Finacle_Provision_Stg_Daily P ON  A.NCIF_Id=P.ENT_CIF
												 ...
												 AND CAST( P.AS_ON_DATE AS date) = @Exec_Date
INNER JOIN IBL_ENPA_DB.dbo.DIM_STD_ASSET_CAT B ON  B.EffectiveFromTimeKey<=@PrevMonthEndTimekey
                                                 AND B.EffectiveToTimeKey>=@PrevMonthEndTimekey
												 AND P.STD_ASSET_CAT=B.STD_ASSET_CATShortNameEnum
												WHERE SrcSysAlt_Key IN (@SrcSysAlt_Key,50)--ADDED ON 20231109 INCLUDING FOR ECBF --ON PROD 20240117
												and isnull(@PrevMoc_Freeze,'') in ('','N')

/* COBORROWER CURNT INSERT FROM Stage Added ON 20231026 FOR COBORROWER BY ZAIN --ON PROD 20240117*/
 INSERT INTO [CoBorrowerData_Curnt](
 AsOnDate
...
,NCIFID_COBORROWER
 )SELECT AS_ON_DATE,SOURCE,ENT_CIF,SRC_CIF,ACC_NO,COBO_ENT_CIF
 FROM IBL_ENPA_STGDB.DBO.FINACLE_COBORROWER_STG_DAILY
...
WHERE [SP_Name]='Merg_Stg_Finacle_Data' AND [EXT_DATE]=@EXT_DT AND ISNULL([Audit_Flg],0)=0
