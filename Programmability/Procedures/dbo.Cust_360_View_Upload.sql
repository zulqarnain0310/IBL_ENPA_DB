...
--GO
CREATE Proc [dbo].[Cust_360_View_Upload]
  @XmlDocument  XML
 ,@TimeKey INT
AS
...
--  ,@TimeKey INT = 26084
   ...
   ,c.value('./AccountNumber[1]','varchar(20)')CustomerACID
	...
	------- Implement Logic By Satwaji as on 21/10/2021 to Show the AsOnDate at the First Column for Excel Sheet Download Data ----
	DECLARE @AsOnDate VARCHAR(10)
		SET @AsOnDate=(SELECT CONVERT(VARCHAR(10),Date,103) FROM SysDaymatrix WHERE TimeKey=@TimeKey)
	...
	ISNULL(CONVERT(VARCHAR(10),A.DCCO_Date,103),'') AS [DCCO Date],
	--'' AS [DCCO Date],
	A.IsARC_Sale AS ARC,
