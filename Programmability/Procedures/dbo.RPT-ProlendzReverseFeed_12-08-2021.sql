...
------------------------===============Prolendz Reverse Feed=======================================-------
/*
CREATED BY  : VEDIKA
CREATED DATE: 18-06-2018
DESCRIPTION : Prolendz REVERSE FEED
*/

CREATE PROC [dbo].[RPT-ProlendzReverseFeed_12-08-2021]
	@TimeKey AS INT,
	@COST AS FLOAT
AS
...
AND MOC_AssetClassAlt_Key IS NOT NULL and SrcSysAlt_Key=60 and MOC_Status='Y' AND MOC_AssetClassAlt_Key<>1
ORDER BY NCIF_Id
...
WHERE PerculatedAccounts.SrcSysAlt_Key=60	
...
WHERE #MOCDATA.SrcSysAlt_Key=60	
AND PreProcessingFreeze='Y'
AND PreProcessingFreezeBy IS NOT NULL
AND PreProcessingFreezeDate IS NOT NULL
AND MOC_Freeze='Y'
