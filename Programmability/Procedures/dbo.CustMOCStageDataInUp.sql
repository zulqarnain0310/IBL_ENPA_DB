...
CREATE PROCEDURE  [dbo].[CustMOCStageDataInUp]  --@TimeKey=27266,@UserLoginID=N'iblfm8840',@OperationFlag=N'1',@MenuId=N'97',@AuthMode=N'Y',@filepath=N'Customer_MOC_Uploadv6.xlsx',@EffectiveFromTimeKey=27266,@EffectiveToTimeKey=27266,@UniqueUploadID=NULL
...
SET DATEFORMAT DMY
	...
	/*SETTING LIVE TIMEKEY*/
		...
		DECLARE @EXT_DATE_LIVE DATE =(SELECT DATE FROM  SYSDATAMATRIX WHERE TIMEKEY=(SELECT DISTINCT EFFECTIVEFROMTIMEKEY FROM NPA_INTEGRATIONDETAILS 
								WHERE EFFECTIVEFROMTIMEKEY NOT IN (SELECT TIMEKEY FROM SYSDATAMATRIX WHERE (CurrentStatus_MOC='C'))
								and EffectiveFromTimeKey=EffectiveToTimeKey))  ---- Changed by liyaqat on 20240801
...
--	set identity_insert HIST_CustMOCUpload_stg on
	/*--ADDED ON 20230916 FOR MOC NOT PREVAILING*/
		...
		LEFT JOIN DimMocReason  R ON R.MocReasonName= A.MOC_Reason
				AND MocReasonCategory='Customer Reason'          -----Added by jaydev on 01062024 MOC Bug
...
/*--ADDED BY ZAIN 20240315 MOC  DATA NOT PREVIALING OBSERVATION*/
		...
		--Distinct
			...
			,GETDATE() AS DateCreated
			--,CASE WHEN A.CustomerId IS NOT NULL THEN 'Y' ELSE 'N' END AS IsUpload--ON UAT 20230621 as isupload was not updatingON PRODUCTION 20230901
			,N.FlgMOC			--ON UAT 20230621 as isupload was not updatingON PRODUCTION 20230901
			,A.LossDate --ADDED BY ZAIN 20231106 MOC  DATA NOT PREVIALING OBSERVATION
			,A.DBTDate --ADDED BY ZAIN 20231106 MOC  DATA NOT PREVIALING OBSERVATION			 
			--,A.MOC_Reason
		--FROM CustMOCUpload_stg A
		...
		LEFT JOIN DimAssetClass B ON B.AssetClassShortName = S.MOC_AssetClassification
				AND B.EffectiveFromTimeKey<=@Timekey_live and B.EffectiveToTimeKey>=@Timekey_live 
		LEFT JOIN DimAssetClass E ON E.AssetClassAlt_Key = N.AC_AssetClassAlt_Key
				AND E.EffectiveFromTimeKey<=@Timekey_live and E.EffectiveToTimeKey>=@Timekey_live 
		LEFT JOIN DimMocReason  R ON R.MocReasonName= A.MOC_Reason
				AND MocReasonCategory='Customer Reason'
				AND R.EffectiveFromTimeKey<=@Timekey_live and R.EffectiveToTimeKey>=@Timekey_live 
...
--------------------------------------------
	...
	BEGIN
		...
		UPDATE 
			...
			DROP TABLE IF EXISTS #CUSTOMER_CAL1		
			DROP TABLE IF EXISTS #CUSTOMER_CAL2


						SELECT A.* INTO #CUSTOMER_CAL1 
						...
						SELECT * INTO #CUSTOMER_CAL
						FROM #CUSTOMER_CAL1
						UNION ALL 
						SELECT * FROM #CUSTOMER_CAL2
...
/*INSERTING RECORDS CHANGED DUE  TO MOC 20240305*/
			...
			SELECT 
				...
				,GETDATE() AS MOC_Date		--CASE WHEN B.MOC_Date IS NULL THEN A.MOC_Date ELSE B.MOC_Date END AS MOC_Date -- Added by Liyaqat on 20240920 for observation raised by dayanand
				...
				,A.SECTOR
				,A.ACMOC_ReasonAlt_Key
				--,'Y' AS FlgMOC   -- COMMENTED DUE TO MOC OBSERVATION WHERE FLGMOC 'Y' ON CURRENT TIMEKEY(ZAIN & MOHIT 29/03/2025)
				,NULL AS FlgMOC
...
PRINT @TIMEKEY
UPDATE NI set
			NCIF_AssetClassAlt_Key=Case when  IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),@SUB_Year)>@EXT_DATE
												then 2  
	  										when  IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),@DB1_Year+@SUB_Year) > @EXT_DATE
												then 3
											when IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),(@DB1_Year+@DB2_Year+@SUB_Year)) > @EXT_DATE
												then 4
											when  IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),(@DB1_Year+@DB2_Year+@SUB_Year)) <= @EXT_DATE
												then 5
				...
				WHERE ISNULL(CONVERT(DATE,RES.MOC_NPADATE,103),'')<>''
					...
					--AND ISNULL(RES.DBTDate,'') =''--20240313 by zain	DECLARE @Ext_Date DATE=(select Date from IBL_ENPA_DB.dbo.SysDataMatrix  where TimeKey=27028)
					AND RES.MOC_AssetClassification<>'LOS'


UPDATE NI set
			NCIF_AssetClassAlt_Key=Case when IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.DBTDate,103),@DB1_Year) > @EXT_DATE
												then 3
											when IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.DBTDate,103),(@DB1_Year+@DB2_Year)) > @EXT_DATE
												then 4
											when  IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),(@DB1_Year+@DB2_Year)) <= @EXT_DATE
												then 5
												else NI.NCIF_AssetClassAlt_Key
												end
...
UPDATE NI set
				NCIF_AssetClassAlt_Key=Case when  IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),@SUB_Year)>@EXT_DATE_LIVE
												then 2  
	  										when  IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),@DB1_Year+@SUB_Year) > @EXT_DATE_LIVE
												then 3
											when IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),(@DB1_Year+@DB2_Year+@SUB_Year)) > @EXT_DATE_LIVE
												then 4
											when  IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),(@DB1_Year+@DB2_Year+@SUB_Year)) <= @EXT_DATE_LIVE
												then 5
												else NI.NCIF_AssetClassAlt_Key
...
UPDATE NI set
			NCIF_AssetClassAlt_Key=Case when IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.DBTDate,103),@DB1_Year) > @EXT_DATE_LIVE
												then 3
											when IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.DBTDate,103),(@DB1_Year+@DB2_Year)) > @EXT_DATE_LIVE
												then 4
											when  IBL_ENPA_DB.[dbo].[GetLeapYearDate] (CONVERT(DATE,RES.MOC_NPADATE,103),(@DB1_Year+@DB2_Year)) <= @EXT_DATE_LIVE
												then 5
												else NI.NCIF_AssetClassAlt_Key
												end
				,NI.DbtDT=RES.DBTDate
...
--CREATED HISTORY TABLE FOR HIST_CustMOCUpload_stg UPLOAD 
	...
	UPDATE UploadCustMOC SET MOC_AssetClassification = (CASE WHEN MOC_AssetClassification = 'STD' THEN '1'
...
UPDATE NID
	...
	--NULLIFYING IN MAIN TABLE NPA_IntegrationDetails in daily data THOSE RECORDS WHICH ARE UPGRADE DUE TO MOC FOR STANDARD
UPDATE NID
	SET   NID.LossDT=NULL /* For erosion process it should not be initailsed on next day commented as on date 09052023.Production deployment 20230603 */ 
		 ,NID.DbtDT=NULL /* For erosion process it should not be initailsed on next day commented as on date 09052023.Production deployment 20230603 */ 
		 ,NID.FlgErosion=NULL /* For erosion process it should not be initailsed on next day commented as on date 09052023.Production deployment 20230603 */ 
	...
	FROM NPA_IntegrationDetails NID INNER JOIN #UPGRADE_DUE_TO_CUSTMOC UP ON NID.CustomerACID=UP.CustomerACID
				AND UP.EffectiveFROMTimeKey<=@TIMEKEY
				AND UP.EffectiveToTimeKey>=@TIMEKEY
	WHERE NID.EffectiveFROMTimeKey<=@TIMEKEY_LIVE
	AND   NID.EffectiveToTimeKey>=@TIMEKEY_LIVE
	AND NID.NCIF_AssetClassAlt_Key NOT IN (7)
