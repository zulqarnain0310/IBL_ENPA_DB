...
create PROCEDURE [dbo].[ProvisionComputation_23_06_2022] (@TimeKey Smallint,@IS_MOC CHAR(1)='N')
WITH RECOMPILE
AS
...
Left Join CURDAT.AcceleratedProv B ON 
--A.NCIF_Id=B.NCIF_Id And A.CustomerId=B.CustomerId And ------04032022 addional provision change
...
INNER JOIN [CurDat].AcceleratedProv AP ON NID.EffectiveFromTimeKey<=@TimeKey
					...
					--		 AND NID.NCIF_Id=AP.NCIF_Id
						---	 AND NID.CustomerId=AP.CustomerId------------------04032022 addional provision change
