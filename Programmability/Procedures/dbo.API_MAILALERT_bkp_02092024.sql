...
--------Alter Table API_MAIL_XML_TABLE Alter column Msg	varchar (200)  


CREATE PROCEDURE [dbo].[API_MAILALERT_bkp_02092024]--'IBL_ENPA_EXTRACTION STEP : Ganaseva_Provision_Stg_Daily' ,'Dear Ganaseva','Ganaseva ETL Staging Extraction was completed Successfully','Ganaseva ETL Staging Extraction was Failed',80,'RF'
...
Begin
	...
	IF @FEEDTYPE='FF'
			Begin 
				...
				Declare @DS_1 Varchar(10)='DS1='+(select cast(ISNULL(Count,0) as Varchar(20)) from #Count_FF where DataSet='DS1')
				Declare @DS_2 Varchar(10)='DS2='+(select cast(ISNULL(Count,0) as Varchar(20)) from #Count_FF where DataSet='DS2')
				Declare @DS_3 Varchar(10)='DS3='+(select cast(ISNULL(Count,0) as Varchar(20)) from #Count_FF where DataSet='DS3')
				Declare @DS_4 Varchar(10)='DS4='+(select cast(ISNULL(Count,0) as Varchar(20)) from #Count_FF where DataSet='DS4')
				Declare @DS_5 Varchar(10)='DS5='+(select cast(ISNULL(Count,0) as Varchar(20)) from #Count_FF where DataSet='DS5')
				Declare @DS_6 Varchar(10)='DS6='+(select cast(ISNULL(Count,0) as Varchar(20)) from #Count_FF where DataSet='DS6')

				Declare @CountString Varchar(200)= 
	...
	IF @FEEDTYPE='RF'
			Begin 
				...
				Declare @CountString_RF Varchar(200)='Record Count for '+@SourceName+' is '+ISNULL(@Count,'')
...
------EXEC sp_OAMethod @Object, 'Open', NULL, 'POST', '--https://emailuat.indusind.com/EmailApi/api/SendEmail', 'false'
EXEC sp_OAMethod @Object, 'Open', '', 'POST', 'https://ealert.indusind.com:1443/EmailApi/api/SendEmail', 'false'  ---- URL changed by Liyaqat on 20240824
EXEC sp_OAMethod @Object, 'SETRequestHeader', '', 'Content-Type', 'application/XML'--TOCHECK

DECLARE @len int
SET @len = len(@body)


EXEC sp_OAMethod @Object, 'SETRequestBody', '', 'Body', @body
EXEC sp_OAMethod @Object, 'Send', '', @body
...
Else 
		...
		DROP TABLE IF EXISTS #2temp_email ;
		select 
		'<Row>
		<RefId>RefId1111</RefId>
		<Txncode>EA0182</Txncode>
		...
		'B2 into #2temp_email from [DimSourceSystemMail] a WHERE SourceAlt_Key= @SOURCEALT_KEY 
		AND SourceSystemMailValidCode='Y'--ADDED ON 2023-07-25
		...
		IF @FEEDTYPE='FF'
		BEGIN
		UPDATE  #2temp_email SET B1= REPLACE(B1,'EA0182','EA0193')
		END


		DECLARE @2Body AS VARCHAR(MAX) 
		DROP TABLE IF EXISTS #2BODY ;
		SELECT (B1+cast(Emailid as varchar(max))+B2) AS XML_COL INTO #2BODY FROM #2temp_email
		SET @2Body = (select  '<Root>
		<ChnlId>D2K</ChnlId>
		<Key>D2K#20230622</Key>'+STRING_AGG(XML_COL,',')+'</Root>' AS BODY  FROM #2BODY)
		...
		EXEC sp_OAMethod @Object, 'SETRequestBody', '', 'Body', @2body
		EXEC sp_OAMethod @Object, 'Send', '', @2body

		select @2body "XML PASSED"
		EXEC sp_OAMethod @Object, 'ResponseText', @ResponseText OUTPUT
		print concat('object : ',@Object)
		print concat('RESPONSE : ',+@ResponseText)
		select @ResponseText "RESPONSE RECEIVED"
		...
		IF @ResponseText LIKE '%Successful%' 
		BEGIN 
			PRINT 'EXECUTING [dbo].[USP_READXMLString]@2body '
			EXEC [dbo].[USP_READXMLString]@2body,@MSG1,@MSG2 
			update A set A.ResponseText = 'Successful' 
				FROM API_MAIL_XML_TABLE A
					inner join DimSourceSystemMail B on A.Emailid=B.SourceSystemMailID
					where A.Emailid in( select replace(replace(cast(Emailid as varchar(max)),'<Emailid>',''),'</Emailid>','') from #2temp_email)
										and B.SourceSystemMailValidCode='Y'
										AND A.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND A.EFFECTIVEFROMTIMEKEY>=@TIMEKEY
										AND A.ResponseText IS NULL
		...
		BEGIN 
			PRINT 'EXECUTING [dbo].[USP_READXMLString]@2body '
			EXEC [dbo].[USP_READXMLString] @2body,@MSG1,@MSG3
			update A set ResponseText = 'Failure'
					FROM API_MAIL_XML_TABLE A
					inner join DimSourceSystemMail B on A.Emailid=B.SourceSystemMailID
					where A.Emailid in( select replace(replace(cast(Emailid as varchar(max)),'<Emailid>',''),'</Emailid>','') from #2temp_email)
