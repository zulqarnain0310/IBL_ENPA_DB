...
GO
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_CustMOCUpload_21092021] 
@MenuID INT=10,  
...
BEGIN
	  ...
	  -- IF OBJECT_ID('tempdb..#UploadCustMOC') IS NOT NULL  
	  --BEGIN  
  ...
  IF NOT (EXISTS (SELECT * FROM CustMOCUpload_stg where filname=@FilePathUpload))
...
SET DATEFORMAT DMY
 ...
 PRINT 'DUB42'
 Declare @count int
 select  @count = max(SrNo)  from UploadCustMOC GROUP BY AsOnDate 
...
UPDATE UploadCustMOC
 ...
 --              THEN CASE WHEN CONVERT(date,AsOnDate) <> CONVERT(date,(Select DATE FROm SysDataMatrix WHERE CurrentStatus_MOC='C')) 
	--		             THEN 1 ELSE 0 END 
	--		   ELSE 2 END)=1
 AND V.AsOnDate NOT IN(Select DATE FROm SysDataMatrix WHERE CurrentStatus_MOC='C')
 --WHERE  (CASE WHEN ISNULL(AsOnDate,'')<>'' AND  ISDATE(AsOnDate)=1 
 --              THEN CASE WHEN CONVERT(date,AsOnDate) <> CONVERT(date,(Select MonthLastDate FROm SysDataMatrix WHERE CurrentStatus_MOC='C')) 
	--		             THEN 1 ELSE 0 END 
	--		   ELSE 2 END)=1
 ...
 --ROW ,UploadID,AsOnDate FROM UploadCustMOC
 --)X
 --WHERE ROW=1
...
---------------- VALIDATIONS ON Source_System_CIF_Customer_Identifier (CustomerID)
...
--								----),1,1,'')   
 ...
 FROM UploadCustMOC V  
 WHERE ISNULL(CustomerId,'') <>'' and LEN(CustomerId)>20
...
--								----),1,1,'')   
 ...
 --UPDATE UploadCustMOC
	--SET  
 --       ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN	'MOC AssetClassification is mandatory. Kindly check and upload again' 
	--					ELSE ErrorMessage+','+SPACE(1)+ 'MOC AssetClassification is mandatory. Kindly check and upload again'	END
	--	,ErrorinColumn='MOC_AssetClassification'    
	--	,Srnooferroneousrows=''
	--FROM UploadCustMOC V  
...
--								----),1,1,'')   
 ...
 --UPDATE UploadCustMOC
	--SET  
 --       ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'MOC NPA Date Can not be Blank . Please enter the MOC NPA Date and upload again'     
	--					ELSE ErrorMessage+','+SPACE(1)+ 'MOC NPA Date Can not be Blank. Please enter the MOC NPA Date and upload again'      END
...
--								--),1,1,'') 
 FROM UploadCustMOC V  
 WHERE ISNULL(V.MOC_AssetClassification,'') IN('SUB','DB1','DB2','DB3','LOS','WO') AND (V.MOC_NPADate)=''
...
--								--),1,1,'') 
 ...
 --      ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'Additional Provision Percentage cannot be blank. Please check the values and upload again'     
	--					ELSE ErrorMessage+','+SPACE(1)+ 'Additional Provision Percentage cannot be blank. Please check the values and upload again'      END
...
-- FROM UploadCustMOC V  
-- WHERE ISNULL(Additionalprovisionpercentage,'')<>''
--AND (CHARINDEX('.',ISNULL(Additionalprovisionpercentage,''))>0  AND Len(Right(ISNULL(Additionalprovisionpercentage,''),Len(ISNULL(Additionalprovisionpercentage,''))-CHARINDEX('.',ISNULL(Additionalprovisionpercentage,''))))<>2)

 -- UPDATE UploadCustMOC
	--SET  
 --       ErrorMessage= CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'InvalidAdditionalprovisionpercentage. Please check the values and upload again'     
	--					ELSE ErrorMessage+','+SPACE(1)+ 'Additionalprovisionpercentage. Please check the values and upload again'     END
 ...
 --  FROM UploadCustMOC V  
 --  WHERE (CHARINDEX('.',Additionalprovisionpercentage))>0
