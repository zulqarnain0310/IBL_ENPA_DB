...
CREATE PROC [dbo].[Merg_Stg_Securitize_Data]
WITH RECOMPILE
AS
...
DECLARE @SrcSysAlt_Key Smallint=(SELECT SourceAlt_Key FROM [dbo].[DimSourceSystem] WHERE SourceName='Finacle2' AND  EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
...
DECLARE @PrevMonthEndDate DATE=(SELECT EOMONTH(DATEADD(MONTH,-1,@Exec_Date)))--ADDED ON UAT 20230612 DEPLOYED ON PROD 20230707 FOR STD PROVISION
DECLARE @PrevMonthEndTimekey Smallint=(SELECT TimeKey FROM SysDataMatrix WHERE CAST(DATE AS DATE)=@PrevMonthEndDate)--ADDED ON UAT 20230612 DEPLOYED ON PROD 20230707 FOR STD PROVISION
DECLARE @PrevMoc_Freeze varchar(5)=(SELECT MOC_Freeze FROM SysDataMatrix WHERE TimeKey=@PrevMonthEndTimekey)--ADDED ON UAT 20230612 DEPLOYED ON PROD 20230707 FOR STD PROVISION



DECLARE @Daily_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Securitize_Stg_Daily_Error)
DECLARE @Incremental_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Securitize_Stg_Incremental_Error)
DECLARE @Security_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Securitize_Stg_Security_Error)
DECLARE @Restructure_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Securitize_Stg_Restructure_Error)
...
SELECT @DataProcessingFlg=(CASE WHEN @Incremental_COUNT>0 OR @Daily_COUNT>0 OR @Security_COUNT>0  OR @Restructure_COUNT>0
                                   OR  (SELECT ISNULL(COUNT(1),0) FROM IBL_ENPA_STGDB.DBO.Securitize_Stg_Daily WHERE AsOnDate=@Exec_Date)=0
                                    THEN 0
                               ELSE 1
                           END)  
...
INSERT INTO [dbo].[LagDateDataDetails]
           ...
           ,[SourceSystem]
           ,[DataDateUsed])
SELECT @Exec_Date,'Securitize',@DATEDATAUSE
END
--Audit 
DELETE IBL_ENPA_STGDB.[dbo].[Procedure_Audit] 
WHERE [SP_Name]='Merg_Stg_Securitize_Data' AND [EXT_DATE]=GETDATE() AND ISNULL([Audit_Flg],0)=0

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_Audit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Merg_Stg_Securitize_Data',GETDATE()
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_NCIF_Update',GETDATE()
...
INNER JOIN IBL_ENPA_STGDB.dbo.Securitize_Stg_Incremental B ON A.EffectiveFromTimeKey<=@TimeKey
...
FROM NPA_IntegrationDetails A
INNER JOIN IBL_ENPA_STGDB.dbo.Securitize_Stg_Daily B ON A.EffectiveFromTimeKey<=@TimeKey
                                                 AND A.EffectiveToTimeKey>=@TimeKey
												 AND B.AsONDATE=@Exec_Date
...
WHERE [SP_Name]='Securitize_NCIF_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit Delete Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_Delete_ClosedAccounts',GETDATE()
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_Delete_ClosedAccounts' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_DailyExisting_Update',GETDATE()
...
INNER JOIN IBL_ENPA_STGDB.dbo.Securitize_Stg_Daily PSD ON NPA.SrcSysAlt_Key=@SrcSysAlt_Key
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_DailyExisting_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_InsertNewAccounts',GETDATE()
...
SELECT PSD.UCIF_ID, 
	   ...
	   'SSIS USER',
	   GETDATE()
FROM  IBL_ENPA_STGDB.dbo.Securitize_Stg_Daily PSD 
...
FROM NPA_IntegrationDetails NPA
INNER JOIN IBL_ENPA_STGDB.dbo.Securitize_Stg_Incremental PSD ON NPA.SrcSysAlt_Key=@SrcSysAlt_Key
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_InsertNewAccounts' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit Incremental Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_Incremental_Update',GETDATE()
...
INNER JOIN IBL_ENPA_STGDB.dbo.Securitize_Stg_Incremental  FI ON EffectiveFromTimeKey<=@TimeKey
													    AND EffectiveToTimeKey>=@TimeKey
...
LEFT JOIN DimAssetClass DAC ON FI.SrcAssetClass=DAC.Finacle2AssetClassCode
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_Incremental_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_WO_Balance_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_WO_Balance_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit Renewal update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_Renewal_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_Renewal_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit WO Columns update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_WO_Columns_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_AdvAcWODetail_Insert' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
--DECLARE @SrcSysAlt_Key Smallint=(SELECT SourceAlt_Key FROM [dbo].[DimSourceSystem] WHERE SourceName='Securitize' AND  EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
...
LEFT JOIN IBL_ENPA_STGDB.dbo.Securitize_Stg_Security SEC  ON  SEC.COLL_ID=ASD.CollateralID
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_AdvSecurityDetail_Insert',GETDATE()
...
FROM IBL_ENPA_STGDB.dbo.Securitize_Stg_Security SEC
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_AdvSecurityDetail_Insert' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit SecurityEntityID Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_SecurityEntityID_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_SecurityEntityID_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit Security Value Columns Update Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_SecurityValueColumns_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_SecurityValueColumns_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit AdvSecurityValueDetail Insert Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_AdvSecurityValueDetail_Insert',GETDATE()
...
(SELECT COLL_ID,MIN(COLL_AMT) COLL_AMT
 FROM IBL_ENPA_STGDB.dbo.Securitize_Stg_Security
 WHERE AS_ON_DATE=@EXT_DT
 GROUP BY AS_ON_DATE/*,ACC_NO*/,COLL_ID) SEC
 INNER JOIN IBL_ENPA_STGDB.dbo.Securitize_Stg_Security STG ON SEC.COLL_ID=STG.COLL_ID
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_AdvSecurityValueDetail_Insert' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_StkStmtDate_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_StkStmtDate_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit MaxDPD Update Added on 18-07-2021 ----------------

-----Added on 25-12-2021 for review and stockstatement
Update NPA_IntegrationDetails set DPD_Renewals=0,DPD_StockStmt=0,StkStmtDate=null,ReviewDueDt=null
WHERE EffectiveFromTimeKey<=@TimeKey
AND EffectiveToTimeKey>=@TimeKey
AND SrcSysAlt_Key=@SrcSysAlt_Key
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_MaxDPD_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_MaxDPD_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_SecurityPrevValue_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_SecurityPrevValue_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
IF((SELECT COUNT(DISTINCT AsOnDate) FROM IBL_ENPA_STGDB.dbo.Securitize_Stg_Daily)=2
AND EXISTS(SELECT 1 FROM IBL_ENPA_STGDB.dbo.Securitize_Stg_Daily WHERE ASonDate=@MonthEndDate)
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_NPAMonthEnd_Update',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_NPAMonthEnd_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0
...
INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_AdvAcRestructureDetail_Update',GETDATE()
...
FROM IBL_ENPA_STGDB.dbo.Securitize_Stg_Restructure A
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_AdvAcRestructureDetail_Update' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

--------------For Sub Audit AdvAcRestructureDetail Insert Added on 18-07-2021 ----------------

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Securitize_AdvAcRestructureDetail_Insert',GETDATE()
...
--------Update Sub Audit
UPDATE IBL_ENPA_STGDB.[dbo].[Procedure_SubAudit] SET End_Date_Time=GETDATE(),[Audit_Flg]=1 
WHERE [SP_Name]='Securitize_AdvAcRestructureDetail_Insert' AND [EXT_DATE]=@Exec_Date AND ISNULL([Audit_Flg],0)=0

END

----ADDED ON UAT 20230612 DEPLOYED ON PROD 20230707 FOR STD PROVISION
...
SELECT @Exec_Date as Date_of_Change,getdate() as DateCreated,A.NCIF_Id,A.CustomerId,A.CustomerACID,A.Balance,A.STD_ASSET_CAT_Alt_key,B.STD_ASSET_CATAlt_key,@PrevMoc_Freeze  MOC_status,'Finacle2'
FROM NPA_IntegrationDetails A
INNER JOIN IBL_ENPA_STGDB.dbo.Securitize_Stg_Provision P ON  A.NCIF_Id=P.ENT_CIF
...
,NCIFID_COBORROWER
 ...
 FROM IBL_ENPA_STGDB.DBO.Securitize_Stg_Coborrower
...
WHERE [SP_Name]='Merg_Stg_Securitize_Data' AND [EXT_DATE]=@EXT_DT AND ISNULL([Audit_Flg],0)=0
