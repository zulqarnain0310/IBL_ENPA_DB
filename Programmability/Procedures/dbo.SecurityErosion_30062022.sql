...
GO
create PROCEDURE [dbo].[SecurityErosion_30062022](@TimeKey int)
AS
...
SELECT DISTINCT RefCustomer_CIF, PrincipleOutstanding,Prev_Value,CurrentValue,
(CurrentValue/Case When Isnull(PrincipleOutstanding,0)=0 then 1 else PrincipleOutstanding end )*100 LossErosionPercentage,
(CurrentValue/Case when ISNULL(Prev_Value,0)=0 then 1 else Prev_Value end )*100 DBT_ErosionPercentage,
	   (CASE WHEN (CASE WHEN CurrentValue=0 THEN 0 ELSE (CurrentValue/PrincipleOutstanding) END)*100<10 
	              THEN @LOSS_Alt_Key
             WHEN  
			 --(ISNULL(CurrentValue,0)/(CASE WHEN ISNULL(PrincipleOutstanding,0)<=0 
				--	THEN ISNULL(CurrentValue,0) ELSE ISNULL(PrincipleOutstanding,0) END))*100<50 
			 --  AND 
			   --(CASE WHEN (CASE WHEN Prev_Value IS NULL THEN CurrentValue  ELSE Prev_Value END)=0 
			   ...
			   (CASE WHEN ISNULL(CurrentValue,0)=0 
	    ...
	    (CASE WHEN (CASE WHEN ISNULL(CurrentValue,0)=0 THEN 0  ELSE (CurrentValue/PrincipleOutstanding)END)*100<10 
		       OR 
			   --((ISNULL(CurrentValue,0)/(CASE WHEN ISNULL(PrincipleOutstanding,0)<=0 THEN ISNULL(CurrentValue,0) ELSE ISNULL(PrincipleOutstanding,0) END))*100<50 
		 ...
		 ,(CASE WHEN (CASE WHEN CurrentValue=0 THEN 0 ELSE (CurrentValue/PrincipleOutstanding) END)*100<10 
		  ...
		  --        And NCIF_AssetClassAlt_Key=@SUB_Alt_Key
				...
				(CASE WHEN (CASE WHEN ISNULL(CurrentValue,0)=0 
						THEN 0  ELSE CurrentValue/ (CASE WHEN ISNULL(Prev_Value,0)=0
						THEN 1 ELSE Prev_Value END) END)*100<=50 
			   And NCIF_AssetClassAlt_Key=@SUB_Alt_Key
			          THEN @EXT_Date
...
INTO #Erosion
FROM 
(SELECT RefCustomer_CIF,SUM(Case When ISNULL(ValuationExpiryDate,'1900-01-01')>=@EXT_Date then  CurrentValue Else 0 End) CurrentValue,SUM(Prev_Value) Prev_Value
FROM
(SELECT A.RefCustomer_CIF,A.CollateralID,ValuationExpiryDate,MIN(B.CurrentValue) CurrentValue,MIN(B.Prev_Value) Prev_Value
...
INNER JOIN
(SELECT A.NCIF_Id,MAX(AC_AssetClassAlt_Key) NCIF_AssetClassAlt_Key,SUM(Balance) Balance,SUM(PrincipleOutstanding)PrincipleOutstanding
