...
CREATE PROCEDURE  [dbo].[ChecksumDataInUp_20250403]
	@Timekey INT,
	...
	BEGIN
	 IF (@OperationFlag=1) 
		BEGIN  
				...
				IF EXISTS (select 1 from Dbo.CheckSumData_FF_MOD where EntityID=@EntityID and ISNULL(AuthorisationStatus,'')<>'R')
				BEGIN
							SET @Result=-6
							RETURN @Result 
				END
