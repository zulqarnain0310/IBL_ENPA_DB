...
GO
CREATE PROC [dbo].[AssetClassDegradation_TEST_20240121](@TIMEKEY  INT)
WITH RECOMPILE
AS 
...
DECLARE @CUTOVERDATE DATE='2099-12-31'--ADDED FOR FINACLE PROLEDZ SAME SOURCE  SYSTEM :- PROLENDZ CLASSIFICATION IN D2K 20231110 ON PROD 20240117
...
UPDATE NPA_IntegrationDetails_TEST_20240119
SET NCIF_NPA_Date=CASE WHEN IsFraud='Y' And AC_AssetClassAlt_Key=1  THEN @Ext_Date
   ...
   ,DegReason=(Case WHEN IsFraud='Y' THEN 'Degrade due to Fraud'
                    ---- WHEN DPD_StockStmt=0 THEN 'Degrade due to Stock Statement'-----29102021 dpd zero bug
					 ...
					 ELSE  NULL
				END) 
FROM NPA_IntegrationDetails_TEST_20240119 A
...
----                              ELSE DegReason
----                         END) 
----FROM NPA_IntegrationDetails_TEST_20240119 NI 
...
into #Audit_Finacle
from NPA_IntegrationDetails_TEST_20240119 where EffectiveFromTimeKey<=@TimeKey and EffectiveToTimeKey>=@TimeKey
and SrcSysAlt_Key IN (CASE WHEN @CUTOVERDATE<@DATE THEN @ProlendzSourceAltKey ELSE @finacleSourceAltKey END)--@finacleSourceAltKey--ADDED FOR FINACLE PROLEDZ SAME SOURCE  SYSTEM :- PROLENDZ CLASSIFICATION IN D2K 20231110 ON PROD 20240117
/* START 17062023 DEPLOYMENT FOR RESSTRUCTURE CHANGES*/
...
Update NPA_IntegrationDetails_TEST_20240119 set  
...
--and SrcSysAlt_Key=@finacleSourceAltKey
 ...
 /* If DBT Dt is available , then we need to do Asset Classification ( Logic as implemented for Finacle )
		...
		Select CustomerACID Into #Erosion_Accts   from NPA_IntegrationDetails_TEST_20240119 where EffectiveFromTimeKey<=@TIMEKEY
...
WHERE --B.AssetClassShortName NOT IN('LOS')
 ...
 NPA_IntegrationDetails_TEST_20240119 b
 on a.EffectiveFromTimeKey<=@TIMEKEY and a.EffectiveToTimeKey>=@TIMEKEY
 and b.EffectiveFromTimeKey<=@TIMEKEY and b.EffectiveToTimeKey>=@TIMEKEY
Delete from AuditTrailAssetClassChange where SrcSysAlt_Key IN (CASE WHEN @CUTOVERDATE<@DATE THEN @ProlendzSourceAltKey ELSE @finacleSourceAltKey END)--@finacleSourceAltKey--ADDED FOR FINACLE PROLEDZ SAME SOURCE  SYSTEM :- PROLENDZ CLASSIFICATION IN D2K 20231110 ON PROD 20240117
 and EffectiveFromTimeKey<=@TIMEKEY and EffectiveToTimeKey>=@TIMEKEY
...
from NPA_IntegrationDetails_TEST_20240119 A where A.SrcSysAlt_Key=@SourceAltKey and ISNULL(A.MaxDPD,0)>=91   -----------A.dpd_overdueloans>90
AND  A.EffectiveFromTimeKey<=@TIMEKEY AND A.EffectiveToTimeKey>=@TIMEKEY
...
*/
 ...
 --Select NCIF_AssetClassAlt_Key,NCIF_NPA_Date,DbtDT,AC_AssetClassAlt_Key,AC_NPA_Date,PrincOverdueSinceDt,* from NPA_IntegrationDetails_TEST_20240119 where NCIF_Id='69700941'
...
/* added by satish as on date 03052023 to handle 1 ncif restructure and another non restructe asset downgrade purpse */
			...
			Select NCIF_Id into #NCIF_RestDual from NPA_IntegrationDetails_TEST_20240119 where EffectiveFromTimeKey<=@TIMEKEY and EffectiveToTimeKey>=@TIMEKEY
			and isnull(IsRestructured,'')='Y' and AC_AssetClassAlt_Key=1
			intersect
			Select NCIF_Id from NPA_IntegrationDetails_TEST_20240119 where EffectiveFromTimeKey<=@TIMEKEY and EffectiveToTimeKey>=@TIMEKEY
...
AND A.NCIF_Id NOT IN(SELECT DISTINCT NCIF_Id FROM NPA_IntegrationDetails_TEST_20240119 WHERE EffectiveFromTimeKey<=@TIMEKEY AND EffectiveToTimeKey<=@TIMEKEY AND IsFraud='Y')
...
----------------------------------------------------------------COBORROWER DEPLOYMENNT START ON PROD 20240117--
			...
			select NCIF_Id,CustomerACID into #CVDMark from NPA_IntegrationDetails_TEST_20240119 A 
				where ( 
...
/*TO UPDATE NATURE OF CLASSIFICATION AS 'C' FOR THE CASES WHICH WE DEGRADED ON OUR END FORALL SOURCE SYSTEM AS REQUESTED BY BANK --CHANGED BY ZAIN ON 20231221*/
	...
	FROM NPA_IntegrationDetails_TEST_20240119 A
			...
			update A set NatureofClassification='C' from NPA_IntegrationDetails_TEST_20240119 A
		...
		update A set NatureofClassification='V' from NPA_IntegrationDetails_TEST_20240119 A
		...
		where EffectiveFromTimeKey<=@TIMEKEY
			...
			update A set ImpactingAccountNo=ImpactingAccount from NPA_IntegrationDetails_TEST_20240119 A
			inner join #CVDMark_Acc B on A.NCIF_Id=B.NCIF_Id
		where EffectiveFromTimeKey<=@TIMEKEY
			AND EffectiveToTimeKey>=@TIMEKEY 
...
/*below code may shifted to the step before acl processing/acl main process */
	...
	FROM [CoBorrowerData_Curnt] a
		LEFT JOIN NPA_IntegrationDetails_TEST_20240119 B
...
FROM NPA_IntegrationDetails_TEST_20240119 A
	...
	UPDATE A
		SET  A.AcDegFlg='Y'
			...
			,A.AcUpgDate=null
	FROM CoBorrowerData_Curnt A
	INNER JOIN NPA_IntegrationDetails_TEST_20240119 B
		ON (B.EffectiveFromTimeKey<=@TIMEKEY AND B.EffectiveToTimeKey>=@TIMEKEY)
		AND A.NCIFID_PrimaryAccount=B.NCIF_Id
		AND A.CustomerACID_PrimaryAccount=B.CustomerACID
	...
	/* AMAR 1102023 - FIND MAX ASSTE CLASS AND MIN NPA DATE FROM PRIMARY BORROWER FOR UPDATE AT COBORROWER LEVEL  */
		DROP TABLE IF EXISTS #CO_BORR_NCIF
			...
			SELECT NCIFID_COBORROWER,
					 CASE WHEN B.FlgErosion ='Y' THEN NULL ELSE MAX(NCIF_AssetClassAlt_Key) END NCIF_AssetClassAlt_Key,----CHANGED ON 20231226 BY ZAIN AS PER OBSERVATION
			...
			FROM CoBorrowerData_Curnt A
				...
				WHERE A.AcDegFlg='Y' 
				--AND AC_AssetClassAlt_Key=1
				and NCIF_AssetClassAlt_Key<>7--added by zain on 20231027
	...
	UPDATE A
	 ...
	 FROM NPA_IntegrationDetails_TEST_20240119 A
		...
		UPDATE NI set
				NCIF_AssetClassAlt_Key=Case when DbtDT is null and IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (NI.NCIF_NPA_Date,@SUB_Year)>@Ext_Date
												then @SubSTD  
	  										when DbtDT is not null and IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (DbtDT,@DB1_Year) > @Ext_Date
												then @DB1
											when DbtDT is not null and  IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (DbtDT,(@DB1_Year+@DB2_Year)) > @Ext_Date
												then @DB2
											when DbtDT is not null and IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (DbtDT,(@DB1_Year+@DB2_Year)) <= @Ext_Date
												then @DB3 
				...
				--			UPDATE NI set
				--NCIF_AssetClassAlt_Key=Case when  IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (NI.NCIF_NPA_Date,@SUB_Year)>@Ext_Date
				--								then @SubSTD  
	  	--									when  IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (NI.NCIF_NPA_Date,@DB1_Year+@SUB_Year) > @Ext_Date
				--								then @DB1
				--							when IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (NI.NCIF_NPA_Date,(@DB1_Year+@DB2_Year+@SUB_Year)) > @Ext_Date
				--								then @DB2
				--							when  IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (NI.NCIF_NPA_Date,(@DB1_Year+@DB2_Year+@SUB_Year)) <= @Ext_Date
				--								then @DB3 
				--								else NI.NCIF_AssetClassAlt_Key
	...
	UPDATE NI set
				NCIF_AssetClassAlt_Key=Case when  IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (NI.NCIF_NPA_Date,@SUB_Year)>@EXT_DATE
												then 2  
	  										when  IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (NI.DbtDT,@DB1_Year) > @EXT_DATE
												then 3
											when IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (NI.DbtDT,(@DB1_Year+@DB2_Year)) > @EXT_DATE
												then 4
											when  IBL_ENPA_DB_LOCAL_DEV.[dbo].[GetLeapYearDate] (NI.DbtDT,(@DB1_Year+@DB2_Year)) <= @EXT_DATE
												then 5
												else NI.NCIF_AssetClassAlt_Key
...
DROP TABLE IF EXISTS #COBORROWER_IMPACTINGACCOUNTS_PRE
	...
	INTO #COBORROWER_IMPACTINGACCOUNTS_PRE
	 FROM [CoBorrowerData_Curnt] A INNER JOIN NPA_IntegrationDetails_TEST_20240119 B
...
DROP TABLE IF EXISTS #COBORROWER_IMPACTINGACCOUNTS_PRE_NOCOBO
	...
	INTO #COBORROWER_IMPACTINGACCOUNTS_PRE_NOCOBO
	 FROM [CoBorrowerData_Curnt] A RIGHT JOIN NPA_IntegrationDetails_TEST_20240119 B
...
SELECT A.NCIFID_COBORROWER,A.SrcSysAlt_Key,A.CustomerACID_PrimaryAccount,B.NCIF_ID NCIFID_PrimaryAccount 
	...
	FROM #COBORROWER_IMPACTINGACCOUNTS_NOCOBO A INNER JOIN NPA_IntegrationDetails_TEST_20240119 B
		ON A.NCIFID_COBORROWER=B.NCIF_ID
		and A.NCIFID_COBORROWER is not null --- added by zain and vinit 20231210
		AND B.EffectiveFromTimeKey<=@TIMEKEY AND B.EffectiveToTimeKey>=@TIMEKEY--20240121
...
/*TO DEGRADE HANDLE CO-BORROWER OF A PRIMARY BORROWER WHERE PRIMARY BORROWER IS WRITE-OFF added on 20231206*/ 
		...
		DROP TABLE IF EXISTS #WRITEOFF_PRIM_COBO_DEG 
			SELECT A.NCIF_Id,A.CustomerACID,B.NCIFID_COBORROWER,A.NCIF_AssetClassAlt_Key,A.AC_NPA_Date INTO  #WRITEOFF_PRIM_COBO_DEG 
					FROM NPA_IntegrationDetails_TEST_20240119 A INNER JOIN CoBorrowerData_Curnt B ON A.CUSTOMERACID=B.CustomerACID_PrimaryAccount
		...
		FROM NPA_IntegrationDetails_TEST_20240119 A INNER JOIN #WRITEOFF_PRIM_COBO_DEG B ON A.NCIF_ID=B.NCIFID_COBORROWER
	...
	/*AGING FOR WO DRAGGED CASES 20240108*/
		...
		UPDATE NI set
				...
				WHERE B.AssetClassShortName NOT IN('LOS','WO')
					AND (ISNULL(NI.FlgProcessing,'N')='N')
					AND NI.DbtDT IS NULL
					AND NI.DegReason = 'DEGRADE DUE TO PRIMARY BORROWER ACCOUNT WO'
...
FROM NPA_IntegrationDetails_TEST_20240119 A INNER JOIN #COBORROWER_IMPACTINGACCOUNTS_FINAL B ON A.NCIF_ID=B.NCIFID_PrimaryAccount
...
UPDATE A SET A.NatureofClassification='C'
			...
			,A.ImpactingAccountNo=COALESCE(B.CUSTOMERACID_PRIMARYACCOUNT,A.CUSTOMERACID)
			,A.DateofImpacting=@Ext_Date
 FROM NPA_IntegrationDetails_TEST_20240119 A INNER JOIN #COBORROWER_IMPACTINGACCOUNTS_FINAL_NOCOBO B ON A.NCIF_ID=B.NCIFID_PrimaryAccount
														--AND A.CUSTOMERACID = B.CUSTOMERACID_COBORROWER
								INNER JOIN  #DEGACS D ON D.NCIF_ID=A.NCIF_ID
								AND A.CustomerACID=D.CustomerACID--ADDED ON 20231110 BY ZAIN
...
UPDATE A SET A.NatureofClassification='D'
			...
			,A.DateofImpacting=@Ext_Date
FROM NPA_IntegrationDetails_TEST_20240119 A INNER JOIN #COBORROWER_IMPACTINGACCOUNTS_FINAL B 
										ON A.NCIF_ID=B.NCIFID_COBORROWER
										AND A.CUSTOMERACID <> B.CUSTOMERACID_COBORROWER
...
AND A.NatureofClassification IS NULL--ADDED 0N 20231031 FOR AN OBSERVARTION
AND A.NCIF_AssetClassAlt_Key not in (1)
...
FROM NPA_IntegrationDetails_TEST_20240119 A INNER JOIN CTE_COBOR_NCIF B 
										ON A.NCIF_ID=B.NCIFID_COBORROWER
										AND A.EffectiveFromTimeKey<=@TIMEKEY AND A.EffectiveToTimeKey>=@TIMEKEY
										--AND ISNULL(A.NatureofClassification,'')<>'C'
										AND A.NatureofClassification is null
										AND A.NCIF_AssetClassAlt_Key not in (1)
										AND A.NatureofClassification IS NULL--ADDED ON 20231221AS OBSERVATION OF MARKING WRONG ACCOUNTS IN IMPACTING ACCOUNT NUMBER
...
where A.NatureofClassification is null
AND A.NCIF_AssetClassAlt_Key not in (1)
...
FROM NPA_IntegrationDetails_TEST_20240119 A INNER JOIN #COBORROWER_IMPACTINGACCOUNTS_FINAL_NOCOBO B 
																	ON A.NCIF_ID=B.NCIFID_COBORROWER
																	AND B.NCIFID_COBORROWER IS NOT NULL
...
select b.CustomerACID_PrimaryAccount,NCIFID_COBORROWER,a.*  INTO #UPDATING_ACCNO_FROM_SRC_AS_C 
			from NPA_IntegrationDetails_TEST_20240119 A inner join CoBorrowerData_Curnt B on a.NCIF_Id=b.NCIFID_COBORROWER
...
UPDATE A SET A.IMPACTINGACCOUNTNO=B.CustomerACID_PrimaryAccount
			...
			from NPA_IntegrationDetails_TEST_20240119 a inner join #UPDATING_ACCNO_FROM_SRC_AS_C b on a.NCIF_Id=b.NCIFID_COBORROWER
			where A.NatureofClassification='D'
			AND A.SrcSysAlt_Key<>100
			AND A.EffectiveFromTimeKey<=@TIMEKEY 
			AND A.EffectiveToTimeKey>=@TIMEKEY
...
UPDATE A SET A.ImpactingSourceSystemName=C.SourceName
			from NPA_IntegrationDetails_TEST_20240119 a inner join #UPDATING_ACCNO_FROM_SRC_AS_C b on a.CustomerACID=b.CustomerACID_PrimaryAccount
...
where NCIF_ID in (
select NCIF_ID from NPA_IntegrationDetails_TEST_20240119 
where NatureofClassification is null 
...
update NPA_IntegrationDetails_TEST_20240119 set NatureofClassification = 'V'
where NCIF_ID in (
select NCIF_ID from NPA_IntegrationDetails_TEST_20240119 
where NatureofClassification is null 
and NCIF_AssetClassAlt_Key not in (1))
...
SELECT CustomerACID,SrcSysAlt_Key INTO #NPA_IntegrationDetails_TEST_20240119 FROM NPA_IntegrationDetails_TEST_20240119 WHERE CustomerACID IN (
SELECT ImpactingAccountNo FROM NPA_IntegrationDetails_TEST_20240119 A WHERE A.EffectiveFromTimeKey<=@TIMEKEY AND A.EffectiveToTimeKey>=@TIMEKEY
)


UPDATE A SET A.ImpactingSourceSystemName=C.SourceName,
			A.ImpactingAccountNo=B.CustomerACID
	FROM NPA_IntegrationDetails_TEST_20240119 A INNER JOIN #NPA_IntegrationDetails_TEST_20240119 B ON A.ImpactingAccountNo=B.CustomerACID
...
SELECT DISTINCT NCIF_Id,CUSTOMERACID,VALUE INTO #ImpactingAccountNo
 FROM NPA_IntegrationDetails_TEST_20240119
	...
	FROM #ImpactingAccountNo A INNER JOIN NPA_IntegrationDetails_TEST_20240119 B ON A.value=B.CustomerACID
	...
	FROM NPA_IntegrationDetails_TEST_20240119 A INNER JOIN  #ImpactingAccountNo_SRCSYSALT_FINAL B ON A.CUSTOMERACID=B.CUSTOMERACID
			WHERE A.EffectiveFromTimeKey<=@TIMEKEY
				AND A.EffectiveToTimeKey>=@TIMEKEY 
