...
GO
CREATE PROC [dbo].[AssetClassDegradation_BKP_20250502] (@TIMEKEY  INT)
WITH RECOMPILE
AS 
...
/*  ADDED BY AMAR ON 17-03-2032 FOR RESSTRUCTURE CHANGES*/
	...
	/*Added by Mayuresh to pull minimum RestructureDt on 20241227*/
	DROP TABLE IF EXISTS #MinRestructureDate
	Select MIN(RestructureDt) RestructureDt,RefCustomerId 
	into #MinRestructureDate
	...
	FROM NPA_IntegrationDetails NI 
	Inner Join #MinRestructureDate RES on NI.CustomerId=RES.RefCustomerId
	WHERE 
		--IsRestructured='Y' AND /*Comented for incorrect Dbtdate for non restructured accounts*/
		ISNULL(RestructureDt,'2099-01-01')<ISNULL(AC_NPA_Date,'2099-01-01')
	/* Comented by Mayuresh on 20241227
	INNER JOIN [CurDat].AdvAcRestructureDetail RES ON NI.EffectiveFromTimeKey<=@TimeKey 
										 AND NI.EffectiveToTimeKey>=@TimeKey 
										 AND RES.EffectiveFromTimeKey<=@TimeKey 
										 AND RES.EffectiveToTimeKey>=@TimeKey 
...
dbtdt=case when --DbtDT IS NULL and --comented by Mayuresh 20241227 for incorrect DBTDT issue
...
--and SrcSysAlt_Key=@finacleSourceAltKey
 ...
 AND (SrcSysAlt_Key IN (SELECT SrcSysAlt_Key FROM #CUTSOURCEALTKEY)--2024-01-27
	 ...
	 /* Mayuresh DBT Date update when ac_npa date is less than ac restructure date Start 20250120*/

	 	DROP TABLE IF EXISTS #MinRestructureAcDate
	Select MIN(A.RestructureDt) RestructureDt,
	MIN(B.AC_NPA_Date) AC_NPA_Date,
	b.NCIF_Id
	
	into #MinRestructureAcDate
	...
	Update A
	Set a.DbtDT = case when b.AC_NPA_Date<b.RestructureDt then IBL_ENPA_DB.[dbo].[GetLeapYearDate](B.AC_NPA_Date,@DB1_Year)
	else IBL_ENPA_DB.[dbo].[GetLeapYearDate](B.RestructureDt,@DB1_Year) end
	From NPA_IntegrationDetails A
	Inner join #MinRestructureAcDate B
	on a.NCIF_Id=b.NCIF_Id

	 /*Mayuresh DBT Date update when ac_npa date is less than ac restructure date End 20250120*/
...
SELECT	     A.NCIF_Id
	...
	Update 	#NCIF_ASSETWriteOff set AC_AssetClassAlt_Key=(Case  
												when WriteOffFlag='Y' and 
													DATEDIFF(day, ISNULL(AC_NPA_Date,WriteOffDate),(select Date from IBL_ENPA_DB.dbo.SysDataMatrix  where TimeKey=@TIMEKEY))between 0 and 365 then @SubSTD
												when WriteOffFlag='Y' and 
													DATEDIFF(day, ISNULL(AC_NPA_Date,WriteOffDate),(select Date from IBL_ENPA_DB.dbo.SysDataMatrix  where TimeKey=@TIMEKEY))between 366 and 730 then @DB1
												when WriteOffFlag='Y' and 
													DATEDIFF(day, ISNULL(AC_NPA_Date,WriteOffDate),(select Date from IBL_ENPA_DB.dbo.SysDataMatrix  where TimeKey=@TIMEKEY))between 731 and 1460 then @DB2
												when WriteOffFlag='Y' and 
													DATEDIFF(day, ISNULL(AC_NPA_Date,WriteOffDate),(select Date from IBL_ENPA_DB.dbo.SysDataMatrix  where TimeKey=@TIMEKEY))between 1461 and 99997 then @DB3
												when WriteOffFlag='Y' and 
													DATEDIFF(day, ISNULL(AC_NPA_Date,WriteOffDate),(select Date from IBL_ENPA_DB.dbo.SysDataMatrix  where TimeKey=@TIMEKEY))=99998 then @LOS
												ELSE AC_AssetClassAlt_Key
												END ) 
...
SET A.NCIF_AssetClassAlt_Key = B.NEWAC_AssetClassAlt_Key
  ...
  ----, A.NatureofClassification='C'---- Line Added for fluctuation observation on 20240521 by LQT for point-6 removed on 20240805
from NPA_IntegrationDetails A
...
----------------------------------------------------------------COBORROWER DEPLOYMENNT START ON PROD 20240117--
			...
			select NCIF_Id,CustomerACID into #CVDMark from NPA_integrationDETAILS A 
				where ( 
...
/*TO UPDATE NATURE OF CLASSIFICATION AS 'C' FOR THE CASES WHICH WE DEGRADED ON OUR END FORALL SOURCE SYSTEM AS REQUESTED BY BANK --CHANGED BY ZAIN ON 20231221*/
	...
	FROM NPA_INTEGRATIONDETAILS A
			...
			update A set NatureofClassification='C' from NPA_integrationDETAILS A
		...
		update A set NatureofClassification='V' from NPA_integrationDETAILS A
		...
		where EffectiveFromTimeKey<=@TIMEKEY
			...
			update A set ImpactingAccountNo=ImpactingAccount from NPA_integrationDETAILS A
			inner join #CVDMark_Acc B on A.NCIF_Id=B.NCIF_Id
		where EffectiveFromTimeKey<=@TIMEKEY
			AND EffectiveToTimeKey>=@TIMEKEY 
...
--/****************** Write off Ac CIF co borrower data update  **********************/ ---- Added for Fluctuation on 20240802 by LQT Point-6
Update A SET  A.AcDegFlg='Y'
			,A.AcDegDate=CASE WHEN NCIF_NPA_Date is not null THEN NCIF_NPA_Date
							  WHEN DateofImpacting is not null THEN DateofImpacting
							ELSE @Ext_Date
							END
			,A.AcUpgFlg='N'
			,A.AcUpgDate=Null
				from CoBorrowerData_Curnt A 
				Inner join NPA_IntegrationDetails C ON A.CustomerACID_PrimaryAccount=C.CustomerACID 
				where  ISNULL(WriteOffFlag,'N')='Y' and ISNULL(A.AcDegFlg,'N')='N' and AcDegDate is NULL 
							and isnull(C.NatureofClassification,'')='C'

--/****************** Write off update end *****************/
	...
	UPDATE A
		SET A.NCIF_AssetClassAlt_Key= CASE WHEN B.NCIF_AssetClassAlt_Key = 6 THEN 2 ELSE B.NCIF_AssetClassAlt_Key END--20240121 OBSRVATION
			,A.NCIF_NPA_Date=CASE WHEN C.NCIF_ID IS NOT NULL AND C.NCIF_NPA_Date<ISNULL(B.NCIF_NPA_Date,CONVERT(DATE,@Ext_Date,103)) --CHANGED BY ZAIN 20231027
									...
									ELSE B.NCIF_NPA_Date--CHANGED BY ZAIN 20240408 FIX-- Added in 12 may UAT testing 
...
DROP TABLE IF EXISTS #COBORROWER_IMPACTINGACCOUNTS_PRE
	...
	INTO #COBORROWER_IMPACTINGACCOUNTS_PRE
	 FROM [CoBorrowerData_Curnt] A INNER JOIN NPA_INTEGRATIONDETAILS B
...
DROP TABLE IF EXISTS #COBORROWER_IMPACTINGACCOUNTS_PRE_NOCOBO
	...
	INTO #COBORROWER_IMPACTINGACCOUNTS_PRE_NOCOBO
	 FROM [CoBorrowerData_Curnt] A RIGHT JOIN NPA_INTEGRATIONDETAILS B
...
SELECT A.NCIFID_COBORROWER,A.SrcSysAlt_Key,A.CustomerACID_PrimaryAccount,B.NCIF_ID NCIFID_PrimaryAccount 
	...
	FROM #COBORROWER_IMPACTINGACCOUNTS_NOCOBO A INNER JOIN NPA_INTEGRATIONDETAILS B
		ON A.NCIFID_COBORROWER=B.NCIF_ID
		and A.NCIFID_COBORROWER is not null --- added by zain and vinit 20231210
...
/*TO DEGRADE HANDLE CO-BORROWER OF A PRIMARY BORROWER WHERE PRIMARY BORROWER IS WRITE-OFF added on 20231206*/ 
		...
		DROP TABLE IF EXISTS #WRITEOFF_PRIM_COBO_DEG 
			SELECT A.NCIF_Id,A.CustomerACID,B.NCIFID_COBORROWER,A.NCIF_AssetClassAlt_Key,A.AC_NPA_Date INTO  #WRITEOFF_PRIM_COBO_DEG 
					FROM NPA_INTEGRATIONDETAILS A INNER JOIN CoBorrowerData_Curnt B ON A.CUSTOMERACID=B.CustomerACID_PrimaryAccount
					WHERE   (A.NCIF_AssetClassAlt_Key IN (7) OR WriteOffFlag='Y') --CHANGED BY ZAIN DUE TO OBSERVATION 20231211/ADDED WRITEOFFFLG LOGIC ON 20231216
						...
						AND A.EffectiveFromTimeKey<=@TIMEKEY AND A.EffectiveToTimeKey>=@TIMEKEY
						AND A.NatureofClassification='C' ---- Added on 20240208 by LQT for Fluctuation point-7
		...
		FROM NPA_INTEGRATIONDETAILS A INNER JOIN #WRITEOFF_PRIM_COBO_DEG B ON A.NCIF_ID=B.NCIFID_COBORROWER
...
FROM NPA_INTEGRATIONDETAILS A INNER JOIN #COBORROWER_IMPACTINGACCOUNTS_FINAL B ON A.NCIF_ID=B.NCIFID_PrimaryAccount
...
UPDATE A SET A.NatureofClassification='C'
			...
			,A.ImpactingAccountNo=COALESCE(B.CUSTOMERACID_PRIMARYACCOUNT,A.CUSTOMERACID)
			,A.DateofImpacting=@Ext_Date
 FROM NPA_INTEGRATIONDETAILS A INNER JOIN #COBORROWER_IMPACTINGACCOUNTS_FINAL_NOCOBO B ON A.NCIF_ID=B.NCIFID_PrimaryAccount
														--AND A.CUSTOMERACID = B.CUSTOMERACID_COBORROWER
								INNER JOIN  #DEGACS D ON D.NCIF_ID=A.NCIF_ID
								AND A.CustomerACID=D.CustomerACID--ADDED ON 20231110 BY ZAIN
...
UPDATE A SET A.NatureofClassification='D'
			...
			,A.DateofImpacting=@Ext_Date
FROM NPA_INTEGRATIONDETAILS A INNER JOIN #COBORROWER_IMPACTINGACCOUNTS_FINAL B 
										ON A.NCIF_ID=B.NCIFID_COBORROWER
										AND A.CUSTOMERACID <> B.CUSTOMERACID_COBORROWER
...
AND A.NCIF_AssetClassAlt_Key not in (1)
AND isnull(A.AC_AssetClassAlt_Key,1)=1 --- Added by Liyaqat  20240822 to exclude self NPA ac from NOC=D
...
FROM NPA_INTEGRATIONDETAILS A INNER JOIN CTE_COBOR_NCIF B 
										ON A.NCIF_ID=B.NCIFID_COBORROWER
										AND A.EffectiveFromTimeKey<=@TIMEKEY AND A.EffectiveToTimeKey>=@TIMEKEY
										--AND ISNULL(A.NatureofClassification,'')<>'C' 
										AND A.NCIF_AssetClassAlt_Key not in (1)
										AND ISNULL(AC_AssetClassAlt_Key,1)=1  --- Added by Liyaqat  on 20240822 to exclude self NPA ac from NOC=D
						...
						where a.NCIF_ID not in ( select NCIF_ID from NPA_INTEGRATIONDETAILS where 
													ISNULL(AC_AssetClassAlt_Key,1)<>1 And ISNULL(Ac_NPA_Date,'2099-01-01')=@Ext_Date )  --- Added by Liyaqat  on 20240822 to exclude self NPA ac from NOC=D
...
AND A.NCIF_AssetClassAlt_Key not in (1)
AND ISNULL(Ac_AssetClassAlt_Key,1)=1 --- Added by Liyaqat  on 20240822 to exclude self NPA ac from NOC=V
...
FROM NPA_INTEGRATIONDETAILS A INNER JOIN #COBORROWER_IMPACTINGACCOUNTS_FINAL_NOCOBO B 
																	ON A.NCIF_ID=B.NCIFID_COBORROWER
																	AND B.NCIFID_COBORROWER IS NOT NULL
...
select b.CustomerACID_PrimaryAccount,NCIFID_COBORROWER,a.*  INTO #UPDATING_ACCNO_FROM_SRC_AS_C 
			from npa_integrationdetails A inner join CoBorrowerData_Curnt B on a.NCIF_Id=b.NCIFID_COBORROWER
...
UPDATE A SET A.IMPACTINGACCOUNTNO=B.CustomerACID_PrimaryAccount
			...
			from npa_integrationdetails a inner join #UPDATING_ACCNO_FROM_SRC_AS_C b on a.NCIF_Id=b.NCIFID_COBORROWER
			where A.NatureofClassification='D'
			AND A.SrcSysAlt_Key<>100
			AND A.EffectiveFromTimeKey<=@TIMEKEY 
			AND A.EffectiveToTimeKey>=@TIMEKEY
...
UPDATE A SET A.ImpactingSourceSystemName=C.SourceName
			from npa_integrationdetails a inner join #UPDATING_ACCNO_FROM_SRC_AS_C b on a.CustomerACID=b.CustomerACID_PrimaryAccount
...
update NPA_integrationDETAILS set NatureofClassification ='V'
where CustomerACID in (select CustomerACID from NPA_integrationDETAILS where NatureofClassification is null and NCIF_AssetClassAlt_Key not in (1))
					AND EffectiveFromTimeKey<=@TIMEKEY 
					AND EffectiveToTimeKey>=@TIMEKEY 
...
SELECT DISTINCT NCIF_Id,CUSTOMERACID,VALUE INTO #ImpactingAccountNo
 FROM NPA_integrationDETAILS
	...
	FROM #ImpactingAccountNo A INNER JOIN NPA_integrationDETAILS B ON A.value=B.CustomerACID
	...
	FROM NPA_integrationDETAILS A INNER JOIN  #ImpactingAccountNo_SRCSYSALT_FINAL B ON A.CUSTOMERACID=B.CUSTOMERACID
			WHERE A.EffectiveFromTimeKey<=@TIMEKEY
				AND A.EffectiveToTimeKey>=@TIMEKEY 
...
--SELECT * INTO NPA_integrationDETAILS_20240121_AFTR_DEG FROM NPA_integrationDETAILS--
---------------------------------------------------------------------UPDATE ENDED	------------------------------------------------------------------
