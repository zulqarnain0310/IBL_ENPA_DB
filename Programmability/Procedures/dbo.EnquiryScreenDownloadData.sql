...
GO
CREATE PROCEDURE [dbo].[EnquiryScreenDownloadData]
...
--DECLARE
--	 @UniqueUploadID INT				= 28
--	,@UploadedBy VARCHAR(50)			= 'ibl55871'
--	,@DateOfUpload VARCHAR(10)			= '07/10/2021'
--	,@AuthorisationLevel	VARCHAR(50)	= 'Approved'
--	,@AuthorisedBy VARCHAR(50)			= 'ibl112680'
--	,@AuthorisedDate VARCHAR(10)		= '07/10/2021'
...
BEGIN
		...
		IF (@UploadType='Account MOC Upload' OR @UploadType='Customer MOC Upload') AND @UniqueUploadID IS NULL
		BEGIN
			Set @Timekey=(select CAST(B.timekey as int) from SysDataMatrix A Inner Join SysDayMatrix B ON A.TimeKey=B.TimeKey 
										where A.CurrentStatus_MOC='C' AND A.MOC_Initialised='Y')
		END
		ELSE IF @UniqueUploadID is not null
		BEGIN 
				Set @Timekey=(select EffectiveFromTimeKey  from ExcelUploadHistory where UniqueUploadID = @UniqueUploadID )
		END
		ELSE
		BEGIN
			Set @Timekey=(select CAST(B.timekey as int) from SysDataMatrix A Inner Join SysDayMatrix B ON A.TimeKey=B.TimeKey 
		...
		BEGIN
				...
				(
					...
					WHERE A.UploadID=@UniqueUploadID
					--AND A.EffectiveFromTimeKey=@Timekey		-- COMMENTED BY SATWAJI as on 12/10/2021 for No Requirement of Timekey
					AND A.IsUpload='Y'
					--AND A.EffectiveFromTimeKey<=@Timekey AND A.EffectiveToTimeKey>=@Timekey
				)A
		END							 
