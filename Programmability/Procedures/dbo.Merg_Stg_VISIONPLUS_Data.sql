...
CREATE PROC [dbo].[Merg_Stg_VISIONPLUS_Data]
WITH RECOMPILE
AS
...
DECLARE @Provision_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.VisionPlus_Provision_Stg_Daily_error)--ADDED ON UAT 20230612 DEPLOYED ON PROD 20230707 FOR STD PROVISION
...
SET [CustomerName]=FI.CustName,
	...
	[AC_AssetClassAlt_Key] = FI.SrcAssetClass,  --- added by satish as on date 31 oct 2022 deployed on 15 dec 2022
	...
	--															END)
	--						      ELSE FI.SrcAssetClass
	--						 END),  ----   Commented by satish as on date 31 Oct 2022 deployed on 15 dec 2022
...
UPDATE NI
  SET NI.DateofImpacting=ISNULL(CONVERT(DATE,FI.DATEOFIMPACTING,103),AC_NPA_Date)
FROM NPA_IntegrationDetails NI
INNER JOIN IBL_ENPA_STGDB.dbo.visionplus_Stg_Incremental  FI ON EffectiveFromTimeKey<=@TimeKey
													    AND EffectiveToTimeKey>=@TimeKey
                                                        AND ISNULL(NI.NCIF_Id,'')=ISNULL(FI.UCIF_ID,'')
...
/* Added New Code by Satish to handle all the vision plus accounts with npa marking as on date 31 Oct 2022 */
UPDATE NI set
[AC_AssetClassAlt_Key] =(CASE WHEN AC_AssetClassAlt_Key=9 THEN @WRE
	                          WHEN AC_AssetClassAlt_Key IN (8,5,4,3,2) 
							                              THEN (CASE when DATEDIFF(DAY,AC_NPA_Date,@Exec_Date)+1 between 0 and 365 then @SubSTD
							                                         when DATEDIFF(DAY,AC_NPA_Date,@Exec_Date)+1 between 366 and 730 then @DB1
							                                         when DATEDIFF(DAY,AC_NPA_Date,@Exec_Date)+1 between 731 and 1460 then @DB2
							                                         when DATEDIFF(DAY,AC_NPA_Date,@Exec_Date)+1 between 1461 and 99997 then @DB3
							                                         when DATEDIFF(DAY,AC_NPA_Date,@Exec_Date)+1 =99998 then @LOS
...
--SELECT * FROM HIST_STD_ASSET_CAT
...
SELECT @Exec_Date as Date_of_Change,getdate() as DateCreated,A.NCIF_Id,A.CustomerId,A.CustomerACID,A.Balance,A.STD_ASSET_CAT_Alt_key,B.STD_ASSET_CATAlt_key,@PrevMoc_Freeze  MOC_status,'VISION PLUS'
FROM NPA_IntegrationDetails A
INNER JOIN IBL_ENPA_STGDB.dbo.VisionPlus_Provision_Stg_Daily P ON  A.NCIF_Id=P.ENT_CIF
