...
CREATE Proc [dbo].[Proc_HistoryWrittenOffData]
(@Date Date)
As
Begin
...
Declare @TimeKey_1 INT =@TimeKey-1
 ...
 select 1,'HistoryWoGradation','NPA Curnt Table Creation',GETDATE(),@Ext_Date,'Y'
...
and WriteOffDate>'2022-12-31'
 ...
 Update SPAuditCheck set endtime=GETDATE() where ID=1 and SPName='HistoryWoGradation' and Ext_Date=@Ext_Date

select 1 


 Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 2,'HistoryWoGradation','restrructure Curnt Table Creation',GETDATE(),@Ext_Date,'Y'
...
AND RefCustomer_CIF IN (SELECT NCIF_Id FROM #npa_integrationdetails_curnt)
 ...
 Update SPAuditCheck set endtime=GETDATE() where ID=2 and SPName='HistoryWoGradation' and Ext_Date=@Ext_Date
select 2


 Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 3,'HistoryWoGradation','insertion into HistoryWrittenOffData',GETDATE(),@Ext_Date,'Y'
...
where NPD.EffectiveFromTimeKey<=@TimeKey and NPD.EffectiveToTimeKey>=@TimeKey -- added by satish as it was creating duplicate accts in the table did as on date 170202023
 ...
 Update SPAuditCheck set endtime=GETDATE() where ID=3 and SPName='HistoryWoGradation' and Ext_Date=@Ext_Date

 
 Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 4,'HistoryWoGradation','NPA archive tbl',GETDATE(),@Ext_Date,'Y'
...
where  arc.EffectiveFromTimeKey<=(Select @Timekey-1) and arc.EffectiveToTimeKey>=(Select @Timekey-1)
 ...
 Update SPAuditCheck set endtime=GETDATE() where ID=4 and SPName='HistoryWoGradation' and Ext_Date=@Ext_Date

select 4


 Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 5,'HistoryWoGradation','restructure archive tbl',GETDATE(),@Ext_Date,'Y'
...
where  arc.EffectiveFromTimeKey<=(Select @Timekey-1) and arc.EffectiveToTimeKey>=(Select @Timekey-1)
 ...
 Update SPAuditCheck set endtime=GETDATE() where ID=5 and SPName='HistoryWoGradation' and Ext_Date=@Ext_Date
...
on #npa_integrationdetails_archive(CustomerAcid)
 ...
 Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 6,'HistoryWoGradation','updating the woff data',GETDATE(),@Ext_Date,'Y'
...
and HOWD.AsonDate=@Ext_Date  ----t-1 date	
 ...
 Update SPAuditCheck set endtime=GETDATE() where ID=6 and SPName='HistoryWoGradation' and Ext_Date=@Ext_Date
END
GO