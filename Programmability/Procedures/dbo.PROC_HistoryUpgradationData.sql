...
CREATE Proc [dbo].[PROC_HistoryUpgradationData]
as
Begin
 ...
 select 1,'HistoryUPGradation','NPA Curnt Table Creation',GETDATE(),@Ext_Date,'Y'
  ...
  where EffectiveFromTimeKey= @Timekey and EffectiveToTimeKey=@Timekey   --- 14 dec 2022
   ...
   Update SPAuditCheck set endtime=GETDATE() where ID=1 and SPName='HistoryUPGradation' and Ext_Date=@Ext_Date

select 1

 Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 2,'HistoryUPGradation','NPA archive Table Creation',GETDATE(),@Ext_Date,'Y'
 ...
 where  EffectiveFromTimeKey=(Select @Timekey-1) and EffectiveToTimeKey=(Select @Timekey-1)
  ...
  Update SPAuditCheck set endtime=GETDATE() where ID=2 and SPName='HistoryUPGradation' and Ext_Date=@Ext_Date

 select 2

  Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 3,'HistoryUPGradation','Index creation',GETDATE(),@Ext_Date,'Y'

Create nonclustered index idx_npa_integrationdetails_archive 
on #npa_integrationdetails_archive(CustomerAcid)

  Update SPAuditCheck set endtime=GETDATE() where ID=3 and SPName='HistoryUPGradation' and Ext_Date=@Ext_Date

    Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 4,'HistoryUPGradation','insertion into HistoryUpgradationData',GETDATE(),@Ext_Date,'Y'
...
Left Join DimsourceSystem ds 
 ...
 and NPD.NCIF_AssetClassAlt_Key=1 and NPDArc.NCIF_AssetClassAlt_Key<>1 
   ...
   Update SPAuditCheck set endtime=GETDATE() where ID=4 and SPName='HistoryUPGradation' and Ext_Date=@Ext_Date

    Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 5,'HistoryUPGradation','additional account for upgradation finding',GETDATE(),@Ext_Date,'Y'
 ...
 and isnull(CustomerACID,'') not in (Select Account from HistoryUpgradationData where AsonDate=@Ext_Date)
  ...
  Update SPAuditCheck set endtime=GETDATE() where ID=5 and SPName='HistoryUPGradation' and Ext_Date=@Ext_Date

    Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 6,'HistoryUPGradation','additional account for upgradation insertion into upgrade',GETDATE(),@Ext_Date,'Y'
...
,NPD.PROJ_COMPLETION_DATE [Project Completion Date]
 ...
 Update SPAuditCheck set endtime=GETDATE() where ID=6 and SPName='HistoryUPGradation' and Ext_Date=@Ext_Date
