...
GO
CREATE PROC [dbo].[AssetClassDegradation_20240121] (@TIMEKEY  INT)
WITH RECOMPILE
AS 
...
where NCIF_ID in (select NCIF_ID from NPA_integrationDETAILS where NatureofClassification is null and NCIF_AssetClassAlt_Key not in (1))
					AND EffectiveFromTimeKey<=@TIMEKEY 
					AND EffectiveToTimeKey>=@TIMEKEY 
					AND NatureofClassification is null
					AND NCIF_AssetClassAlt_Key not in (1)
