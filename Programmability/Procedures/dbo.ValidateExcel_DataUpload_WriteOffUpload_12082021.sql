...
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_WriteOffUpload_12082021] 
@MenuID INT=10,  
...
--								----),1,1,'')   
 ...
 FROM UploadWriteOff V  
 WHERE ISNULL(CustomerAcID,'') LIKE'%[,!@#$%^&*()+=-_/\]%'
...
-----Comparison of WriteOffDate with AsOnDate
 ...
 FROM UploadWriteOff V  
 WHERE  WriteOffType NOT IN('AWO','TWO')
...
UPDATE UploadWriteOff
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'WriteOffType Can not be Blank . Please enter the WriteOffDate and upload again'     
						ELSE ErrorMessage+','+SPACE(1)+ 'WriteOffType Can not be Blank. Please enter the DateOfApproval and upload again'      END
		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'WriteOffType' ELSE   ErrorinColumn +','+SPACE(1)+'WriteOffType' END      
		,Srnooferroneousrows=V.SrNo
		--STUFF((SELECT ','+SRNO 
		--						FROM #UploadNewAccount A
...
-- WHERE ISNULL(DateOfApproval,'')<>'' AND ISDATE(DateOfApproval)=0
 ...
 --SELECT ROW_NUMBER() OVER(PARTITION BY UploadID  ORDER BY  UploadID ) 
 --ROW ,UploadID FROM UploadWriteOff
 --)X
 --WHERE ROW=1
 ...
 --SELECT ROW_NUMBER() OVER(PARTITION BY UploadID  ORDER BY  UploadID ) 
 --ROW ,UploadID,DateOfSaletoARC FROM UploadWriteOff
 --)X
 --WHERE ROW=1
