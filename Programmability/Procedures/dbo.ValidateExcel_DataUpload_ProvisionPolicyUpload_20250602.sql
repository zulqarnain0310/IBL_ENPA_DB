...
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_ProvisionPolicyUpload_20250602] 
@MenuID INT,  
...
-------------validations on Provision percentage From4monthsupto6months-------
 ...
 UPDATE UploadProvisionPolicy
	SET  
		...
		,Srnooferroneousrows=V.SrNo								  
 FROM UploadProvisionPolicy V  
 WHERE ISNULL(From4monthsupto6months,'') LIKE '%[,!@#$%^&*()+=/\]%' --OR ISNULL(From4monthsupto6months,'') LIKE '%!%%' ESCAPE '!' --added by Mayuresh to consider % as string on 20241105
 ...
 UPDATE UploadProvisionPolicy
	SET  
		...
		,Srnooferroneousrows=V.SrNo								  
 FROM UploadProvisionPolicy V  
 WHERE ISNULL(From7monthsupto9months,'') LIKE '%[,!@#$%^&*()+=/\]%' --OR ISNULL(From7monthsupto9months,'') LIKE '%!%%' ESCAPE '!' --added by Mayuresh to consider % as string on 20241105
...
-------------validations on Provision percentage From10monthsupto12months-------
 ...
 UPDATE UploadProvisionPolicy
	SET  
		...
		,Srnooferroneousrows=V.SrNo								  
 FROM UploadProvisionPolicy V  
 WHERE ISNULL(From10monthsupto12months,'') LIKE '%[,!@#$%^&*()+=/\]%' --OR ISNULL(From10monthsupto12months,'') LIKE '%!%%' ESCAPE '!' --added by Mayuresh to consider % as string on 20241105
