...
CREATE PROCEDURE [dbo].[ProvisionComputation_BKP_20230922] (@TimeKey Smallint,@IS_MOC CHAR(1)='N')
WITH RECOMPILE
AS
...
ProvisionAlt_Key=(CASE WHEN IsFITL='Y' THEN @FITL
                       ...
                       WHEN NCIF_AssetClassAlt_Key=@DB2_Alt_Key
		                    THEN (CASE WHEN SrcSysAlt_Key=@Prol and ProductCode IN('H','S','H_R','S_R') And NCIF_NPA_Date>'2020-12-31'  THEN @DB2PROL_70 
							           WHEN SrcSysAlt_Key=@Prol and ProductCode IN('H','S','H_R','S_R') And NCIF_NPA_Date <='2020-12-31'  THEN @DB2PROL 
									   WHEN SrcSysAlt_Key=@Prol and ProductCode IN('G','T_R','G_R','T') And NCIF_NPA_Date>'2020-12-31'  THEN @DB2PROL_60
									    WHEN SrcSysAlt_Key=@Prol and ProductCode IN('G','T_R','G_R','T') And NCIF_NPA_Date <='2020-12-31'  THEN @DB2PROL
									   WHEN SrcSysAlt_Key=@Prol and ProductCode IN('C_R','C') And NCIF_NPA_Date>'2020-12-31'  THEN @DB2PROL_50
									   WHEN SrcSysAlt_Key=@Prol and ProductCode IN('C_R','C') And NCIF_NPA_Date<='2020-12-31'  THEN @DB2PROL
									   ELSE @DB2GEN END)
