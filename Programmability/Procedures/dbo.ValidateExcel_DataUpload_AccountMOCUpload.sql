...
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_AccountMOCUpload] 
@MenuID INT=10,  
...
--								----),1,1,'')   
 ...
 FROM UploadAccountMOC V  
 WHERE ISNULL(CustomerId,'') LIKE'%[,!@#$%^&*()+=-/\]%'--changed on 20230710 DEPLOYED ON PROD 20230712 on UAT by zain
...
--								----),1,1,'')   
 ...
 FROM UploadAccountMOC V  
 WHERE ISNULL(AccountID,'') <>'' and LEN(AccountID)>17--CHANGED ON UAT 20230710 DEPLOYED ON PROD 20230712 BY ZAIN 
...
--								----),1,1,'')   
 ...
 ON V.MOCReason= case when B.MocReasonName='Other reason of Account MOC' then 'Other' else B.MocReasonName END
 AND --(B.EffectiveFromTimeKey<=@Timekey AND
 (B.EffectiveToTimeKey>=@Timekey)
 WHERE --ISNULL(V.MOCReason,'')<>''
 B.MocReasonName IS NULL
-- AND V.SourceSystem NOT IN(SELECT B.SourceName FROM NPA_IntegrationDetails A
-- INNER JOIN DimSourceSystem B
-- ON A.SrcSysAlt_Key=B.SourceAlt_Key
-- AND (B.EffectiveFromTimeKey<=@Timekey AND B.EffectiveToTimeKey>=@Timekey)
...
--)
 ...
 UPDATE UploadAccountMOC
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN  'MOC Reason Not exists in Account category, kindly enter existing reason and upload again'     
						ELSE ErrorMessage+','+SPACE(1)+ 'MOC Reason Not exists in Account category, kindly enter existing reason and upload again'     END
		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'MOCReason' ELSE ErrorinColumn +','+SPACE(1)+  'MOCReason' END  
		,Srnooferroneousrows=V.SrNo
 FROM UploadAccountMOC V  
 WHERE  ISNULL(V.MOCReason,'') NOT IN (select case when MocReasonName='Other reason of Account MOC' then 'Other' else MocReasonName end from DimMocReason 
																																	where MocReasonCategory='Account Reason' AND EffectiveToTimeKey=49999)
 PRINT 'MOC REASON VALIDATION'

 
--------------------------------------------------------------------------------------------------
/*Validations on Remark column added on 12-02-2024 by Shubham Kamble as per CR */
