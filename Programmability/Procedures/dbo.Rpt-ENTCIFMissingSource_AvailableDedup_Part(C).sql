SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO

                    
create  Proc [dbo].[Rpt-ENTCIFMissingSource_AvailableDedup_Part(C)] 
@DtEnter as varchar(20)
,@Cost as Float
,@DimsourceSystem as int
,@NPA AS INT
AS


--DECLARE	
--@DtEnter as varchar(20)='30/09/2017'
--,@Cost AS FLOAT=1000
--,@DimsourceSystem as int=10
--,@NPA AS INT=1


DECLARE @DtEnter1 date ,@From1 date,@to1 date 

SET @DtEnter1=(SELECT Rdate FROM dbo.DateConvert(@DtEnter))
----Print @DtEnter1


DECLARE @TimeKey as int=(select TimeKey from SysDayMatrix where date=@DtEnter1)
----Print @TimeKey 

IF(OBJECT_ID('tempdb..#UPDATED') is not null)
DROP TABLE #UPDATED


SELECT * INTO #UPDATED
FROM
(
SELECT DISTINCT Client_ID,Source_NCIF,ChangedNCIF,NCIF_ValFlag,Timekey
	   FROM NCIF_VALIDATION
	   where NCIF_ValFlag='C'
	   and SrcSysAlt_Key not in(30,50)
	   --AND ChangedNCIF='11341172'
	   AND Timekey=@TimeKey
)TEMP
option(recompile)
CREATE  CLUSTERED INDEX IX_UPDATED ON #UPDATED(ChangedNCIF)

CREATE NONCLUSTERED INDEX IX_UPDATED1 ON #UPDATED(ChangedNCIF)
INCLUDE (Client_ID,Source_NCIF,NCIF_ValFlag,Timekey)


select * into #npa_Integration
From
(
select NCIF_Id,CustomerID,CustomerName,PAN,ProductType,CustomerACID,SanctionedLimit,DrawingPower,MaxDPD,SubSegment,AC_AssetClassAlt_Key
,MOC_AssetClassAlt_Key,SrcSysAlt_Key,Balance,
NCIF_AssetClassAlt_Key,NPA_IntegrationDetails.ProductAlt_Key,DPD_Renewals
 from NPA_IntegrationDetails
 where EffectiveFromTimeKey<=@TimeKey and EffectiveToTimeKey>=@TimeKey
)npa_Integration

option(recompile)
--------------------------AVAILABLE IN DEDUP SYSTEM & MISSING IN SOURCE SYSTEM-------------

SELECT * FROM (
SELECT 
DISTINCT

DimSourceSystem.SourceName										AS 'SourceSystem'

,Source_NCIF													AS 'SOURCEENTCIF'

,ChangedNCIF													AS 'CHANGEDENTCIF'

,NPA_IntegrationDetails.NCIF_Id									AS 'NCIF'


,NPA_IntegrationDetails.CustomerID								AS 'CustomerID'


,NPA_IntegrationDetails.CustomerName							AS 'CustomerName'


,NPA_IntegrationDetails.PAN										AS 'PAN'

,NPA_IntegrationDetails.ProductType								AS 'Facility'

,NPA_IntegrationDetails.CustomerACID							AS 'Account No.'

,ISNULL(NPA_IntegrationDetails.SanctionedLimit,0)/@Cost			AS 'Limit'

,ISNULL(NPA_IntegrationDetails.Balance,0)/@Cost					AS 'Outstanding'

,NCIF_ValFlag

,ISNULL(MOCASSET.AssetClassName,DimAssetClass.AssetClassName)	AS 'ASSETCLASS'

,isnull(NPA_IntegrationDetails.DrawingPower,0)/@Cost	        AS 'DP'

,ISNULL(NPA_IntegrationDetails.DPD_Renewals,0)                  AS 'DPDRenewals'  

,ISNULL(NPA_IntegrationDetails.MaxDPD,0)                        AS 'MaxDPD'

,ISNULL(NPA_IntegrationDetails.SubSegment,'NA')                 AS  'Segment'
,AC_AssetClassAlt_Key
,MOC_AssetClassAlt_Key,
NCIF_AssetClassAlt_Key

,'Populated from Dedup System to Source System'					AS 'Action'

,''                                                             as BillNo

,0                                                              AS 'Outstanding1'  

FROM  #UPDATED NCIF_VALIDATION

INNER JOIN  #npa_Integration NPA_IntegrationDetails		ON NPA_IntegrationDetails.CustomerId=NCIF_VALIDATION.Client_ID
													--AND NPA_IntegrationDetails.EffectiveFromTimeKey<=@TimeKey AND NPA_IntegrationDetails.EffectiveToTimeKey>=@TimeKey
														AND  NPA_IntegrationDetails.ProductAlt_Key<>3200
														AND NPA_IntegrationDetails.AC_AssetClassAlt_Key<>7

														 
INNER JOIN DimSourceSystem				ON  DimSourceSystem.SourceAlt_Key=NPA_IntegrationDetails.SrcSysAlt_Key
										AND DimSourceSystem.EffectiveFromTimeKey<=@TimeKey AND DimSourceSystem.EffectiveToTimeKey>=@TimeKey

LEFT JOIN DimAssetClass				    ON DimAssetClass.AssetClassAlt_Key=NPA_IntegrationDetails.NCIF_AssetClassAlt_Key
										AND DimAssetClass.EffectiveFromTimeKey<=@TimeKey AND DimAssetClass.EffectiveToTimeKey>=@TimeKey

LEFT JOIN DimAssetClass MOCASSET	    ON MOCASSET.AssetClassAlt_Key=NPA_IntegrationDetails.MOC_AssetClassAlt_Key
										AND MOCASSET.EffectiveFromTimeKey<=@TimeKey AND MOCASSET.EffectiveToTimeKey>=@TimeKey	

INNER JOIN DIMPRODUCT					ON DIMPRODUCT.ProductAlt_Key=NPA_IntegrationDetails.ProductAlt_Key
										AND DIMPRODUCT.EffectiveFromTimeKey<=@TimeKey AND DIMPRODUCT.EffectiveToTimeKey>=@TimeKey

WHERE (@NPA=0 AND (  NPA_IntegrationDetails.ProductAlt_Key<>3200)
AND (DimSourceSystem.SourceAlt_Key=@DimsourceSystem OR @DimsourceSystem=0))

OR (@NPA=1
AND (DimSourceSystem.SourceAlt_Key=@DimsourceSystem OR @DimsourceSystem=0)

AND (ISNULL(MOC_AssetClassAlt_Key,NCIF_AssetClassAlt_Key)<>1
	  OR  (ISNULL(MOC_AssetClassAlt_Key,NCIF_AssetClassAlt_Key)=1
     
	      AND 

           CASE WHEN  AgriFlag='N' THEN  (  CASE WHEN isnull(MaxDPD,0)>isnull(DPD_Renewals,0) THEN  (CASE WHEN isnull(MaxDPD,0)>90 THEN 1 END)
								          WHEN isnull(DPD_Renewals,0)>isnull(MaxDPD,0) THEN (CASE WHEN ISNULL(DPD_Renewals,0)>180 THEN 1 END)
									END )
									
                WHEN AgriFlag='Y' THEN  (  CASE WHEN isnull(MaxDPD,0)>isnull(DPD_Renewals,0) THEN  (CASE WHEN isnull(MaxDPD,0)>365 THEN 1 END)
								          WHEN isnull(DPD_Renewals,0)>isnull(MaxDPD,0) THEN (CASE WHEN ISNULL(DPD_Renewals,0)>180 THEN 1 END)
									END )							
											
										END =1 
		)
     )
     




)

union all

select 

DISTINCT 

DimSourceSystem.SourceName										AS 'SourceSystem'

,Source_NCIF													AS 'SOURCEENTCIF'

,ChangedNCIF													AS 'CHANGEDENTCIF'

,B.NCIF_Id									AS 'NCIF'


,B.CustomerID								AS 'CustomerID'


,B.CustomerName							AS 'CustomerName'


,B.PAN										AS 'PAN'

,B.ProductType								AS 'Facility'

,B.CustomerACID							AS 'Account No.'

,ISNULL(B.SanctionedLimit,0)			AS 'Limit'

,ISNULL(B.Balance,0)					AS 'Outstanding'

,NCIF_ValFlag

,ISNULL(MOCASSET.AssetClassName,DimAssetClass.AssetClassName)	AS 'ASSETCLASS'

,isnull(B.DrawingPower,0)	        AS 'DP'

,ISNULL(B.DPD_Renewals,0)                  AS 'DPDRenewals'  

,ISNULL(B.MaxDPD,0)                        AS 'MaxDPD'

,ISNULL(B.SubSegment,'NA')                 AS  'Segment'

,AC_AssetClassAlt_Key
,MOC_AssetClassAlt_Key,
NCIF_AssetClassAlt_Key

,'Populated from Dedup System to Source System'					AS 'Action'

,B.BillNo                                                AS BillNo

,ISNULL(BillDetails.Balance1,0)					AS 'Outstanding1'


from NCIF_VALIDATION A


INNER JOIN NPA_IntegrationBillDetails B on A.ChangedNCIF=B.NCIF_Id
                                        AND B.EffectiveFromTimeKey<=@TimeKey AND B.EffectiveToTimeKey>=@TimeKey
                                        AND A.NCIF_ValFlag='C'
										AND B.SrcSysAlt_Key in(30,50)                                    
                                        --and A.SrcSysAlt_Key IN(30,50)


INNER JOIN #npa_Integration C     ON C.CustomerACID=B.CustomerACID
                                  --AND NCIF_ValFlag='U'
								  --AND A.SrcSysAlt_Key=30     
								  
LEFT JOIN ( select CustomerACID ,sum(Balance)Balance1 from NPA_IntegrationBillDetails
                               				where  NPA_IntegrationBillDetails.EffectiveFromTimeKey<=@TimeKey AND NPA_IntegrationBillDetails.EffectiveToTimeKey>=@TimeKey
                                            --AND A.NCIF_ValFlag='U'
										    AND NPA_IntegrationBillDetails.SrcSysAlt_Key=50 
											GROUP BY CustomerACID )BillDetails
											ON BillDetails.CustomerACID=B.CustomerACID
													 								                                
INNER JOIN DIMPRODUCT					ON DIMPRODUCT.ProductAlt_Key=C.ProductAlt_Key
										AND DIMPRODUCT.EffectiveFromTimeKey<=@TimeKey AND DIMPRODUCT.EffectiveToTimeKey>=@TimeKey

INNER JOIN DimSourceSystem              ON  DimSourceSystem.SourceAlt_Key=B.SrcSysAlt_Key
										AND DimSourceSystem.EffectiveFromTimeKey<=@TimeKey 
										AND DimSourceSystem.EffectiveToTimeKey>=@TimeKey


LEFT JOIN DimAssetClass				    ON DimAssetClass.AssetClassAlt_Key=C.NCIF_AssetClassAlt_Key
										AND DimAssetClass.EffectiveFromTimeKey<=@TimeKey AND DimAssetClass.EffectiveToTimeKey>=@TimeKey

LEFT JOIN DimAssetClass MOCASSET	    ON MOCASSET.AssetClassAlt_Key=C.MOC_AssetClassAlt_Key
										AND MOCASSET.EffectiveFromTimeKey<=@TimeKey AND MOCASSET.EffectiveToTimeKey>=@TimeKey	



WHERE 

(@NPA=0 AND (  C.ProductAlt_Key<>3200)
AND (DimSourceSystem.SourceAlt_Key=@DimsourceSystem OR @DimsourceSystem=0))

OR

 (@NPA=1
AND (DimSourceSystem.SourceAlt_Key=@DimsourceSystem OR @DimsourceSystem=0)

AND (ISNULL(MOC_AssetClassAlt_Key,NCIF_AssetClassAlt_Key)<>1
	  OR  (ISNULL(MOC_AssetClassAlt_Key,NCIF_AssetClassAlt_Key)=1
     
	      AND 

           CASE WHEN  AgriFlag='N' THEN  (  CASE WHEN isnull(C.MaxDPD,0)>isnull(C.DPD_Renewals,0) THEN  (CASE WHEN isnull(C.MaxDPD,0)>90 THEN 1 END)
								          WHEN isnull(C.DPD_Renewals,0)>isnull(C.MaxDPD,0) THEN (CASE WHEN ISNULL(C.DPD_Renewals,0)>180 THEN 1 END)
									END )
									
                WHEN AgriFlag='Y' THEN  (  CASE WHEN isnull(C.MaxDPD,0)>isnull(C.DPD_Renewals,0) THEN  (CASE WHEN isnull(C.MaxDPD,0)>365 THEN 1 END)
								          WHEN isnull(C.DPD_Renewals,0)>isnull(C.MaxDPD,0) THEN (CASE WHEN ISNULL(C.DPD_Renewals,0)>180 THEN 1 END)
									END )							
											
										END =1 
		)
     )
     
)

--GROUP BY 
--DimSourceSystem.SourceName
--,Source_NCIF
--,ChangedNCIF
--,B.NCIF_Id
--,B.CustomerId
--,B.CustomerName
--,B.PAN
--,B.ProductType
--,B.CustomerACID
--,B.SanctionedLimit
--,B.Balance
--,NCIF_ValFlag
--,DimAssetClass.AssetClassName
--,MOCASSET.AssetClassName
--,DimAssetClass.AssetClassName
--,B.DrawingPower
--,B.DPD_Renewals
--,B.MaxDPD
--,B.Segment
--,B.SubSegment
--,C.AC_AssetClassAlt_Key
--,C.MOC_AssetClassAlt_Key
--,C.NCIF_AssetClassAlt_Key
--,B.BillNo





)A

-----/*


----*/
--option(recompile)


DROP TABLE #UPDATED
drop table #npa_Integration
GO