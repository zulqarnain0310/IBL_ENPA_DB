...
CREATE PROC [dbo].[Merg_Stg_Prolendz_Data]
WITH RECOMPILE
AS
...
DECLARE @CUTOVERDATE DATE='2024-01-24'--ON PROD 20240117--CHANGED BY ZAIN ON 2024-01-31
DECLARE @MAX_AccountEntityID INT=(SELECT ISNULL(MAX(AccountEntityID),0) FROM NPA_IntegrationDetails)
DECLARE @SrcSysAlt_Key Smallint=(SELECT SourceAlt_Key FROM [dbo].[DimSourceSystem] WHERE SourceName='Prolendz' AND  EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
...
DECLARE @Daily_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Prolendz_Stg_Daily_Error)
DECLARE @Incremental_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Prolendz_Stg_Incremental_Error)
DECLARE @Security_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Prolendz_Stg_Security_Error)
DECLARE @Restructure_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Prolendz_Stg_Restructure_Error)
DECLARE @PROVISION_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Prolendz_Provision_Stg_Daily_Error) --ADDED ON UAT 20230612 DEPLOYED ON PROD 20230707 FOR STD PROVISION
...
SELECT @DataProcessingFlg=(CASE WHEN @Incremental_COUNT>0 OR @Daily_COUNT>0 OR @Security_COUNT>0  OR @Restructure_COUNT>0
                                   OR  (SELECT ISNULL(COUNT(1),0) FROM IBL_ENPA_STGDB.DBO.Prolendz_Stg_Daily WHERE AsOnDate=@Exec_Date)=0
                                    THEN 0
                               ELSE 1
                           END)  
...
INSERT INTO [dbo].[LagDateDataDetails]
           ...
           ,[SourceSystem]
           ,[DataDateUsed])
SELECT @Exec_Date,'Prolendz',@DATEDATAUSE
END 
--Audit 
DELETE IBL_ENPA_STGDB.[dbo].[Procedure_Audit] 
WHERE [SP_Name]='Merg_Stg_Prolendz_Data' AND [EXT_DATE]=GETDATE() AND ISNULL([Audit_Flg],0)=0

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_Audit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Merg_Stg_Prolendz_Data',GETDATE()
...
INNER JOIN IBL_ENPA_STGDB.dbo.Prolendz_Stg_Incremental B ON A.EffectiveFromTimeKey<=@TimeKey
...
FROM NPA_IntegrationDetails A
INNER JOIN IBL_ENPA_STGDB.dbo.Prolendz_Stg_Daily B ON A.EffectiveFromTimeKey<=@TimeKey
                                                 AND A.EffectiveToTimeKey>=@TimeKey
												 AND B.AsONDATE=@Exec_Date
...
INNER JOIN IBL_ENPA_STGDB.dbo.Prolendz_Stg_Daily PSD ON NPA.SrcSysAlt_Key=@SrcSysAlt_Key
...
SELECT PSD.UCIF_ID, 
	   ...
	   'SSIS USER',
	   GETDATE()
FROM  IBL_ENPA_STGDB.dbo.Prolendz_Stg_Daily PSD 
...
FROM NPA_IntegrationDetails NPA
INNER JOIN IBL_ENPA_STGDB.dbo.Prolendz_Stg_Incremental PSD ON NPA.SrcSysAlt_Key=@SrcSysAlt_Key
...
SET [CustomerName]=FI.CustName,
    NCIF_Id=COALESCE(FI.UCIF_ID,FI.PAN,'PRLNDZ_'+FI.RefCustomerID),
    [PAN]=FI.PAN,
	...
	,NI.NatureofClassification=FI.NATUREOFCLASSIFICATION--20231026 FOR COBORROWER BY ZAIN--ON PROD 20240117
FROM NPA_IntegrationDetails NI
INNER JOIN IBL_ENPA_STGDB.dbo.Prolendz_Stg_Incremental  FI ON EffectiveFromTimeKey<=@TimeKey
													    AND EffectiveToTimeKey>=@TimeKey
...
LEFT JOIN DimAssetClass DAC ON FI.SrcAssetClass=DAC.ProlendzAssetClassCode
...
/*update when finacle and prolendz from same source ADDED ON UAT 20230707*/
IF (@CUTOVERDATE < @Exec_Date)
BEGIN
UPDATE NI
SET [AC_AssetClassAlt_Key] =DAC.AssetClassAlt_Key	
FROM NPA_IntegrationDetails NI
INNER JOIN IBL_ENPA_STGDB.dbo.Prolendz_Stg_Incremental  FI ON EffectiveFromTimeKey<=@TimeKey
													    AND EffectiveToTimeKey>=@TimeKey
                                                        AND ISNULL(NI.NCIF_Id,'')=ISNULL(FI.UCIF_ID,'')
...
LEFT JOIN IBL_ENPA_STGDBDB.dbo.Prolendz_Stg_Security SEC  ON  SEC.COLL_ID=ASD.CollateralID
...
FROM IBL_ENPA_STGDB.dbo.Prolendz_Stg_Security SEC
...
(SELECT COLL_ID,MIN(COLL_AMT) COLL_AMT
 FROM IBL_ENPA_STGDB.dbo.Prolendz_Stg_Security
 WHERE AS_ON_DATE=@EXT_DT
 GROUP BY AS_ON_DATE/*,ACC_NO*/,COLL_ID) SEC
 INNER JOIN IBL_ENPA_STGDB.dbo.Prolendz_Stg_Security STG ON SEC.COLL_ID=STG.COLL_ID
...
FROM IBL_ENPA_STGDB.dbo.Prolendz_Stg_Restructure A
...
SELECT @Exec_Date as Date_of_Change,getdate() as DateCreated,A.NCIF_Id,A.CustomerId,A.CustomerACID,A.Balance,A.STD_ASSET_CAT_Alt_key,B.STD_ASSET_CATAlt_key,@PrevMoc_Freeze  MOC_status,'Prolendz'
FROM NPA_IntegrationDetails A
INNER JOIN IBL_ENPA_STGDB.dbo.Prolendz_Provision_Stg_Daily P ON  A.NCIF_Id=P.ENT_CIF
...
,NCIFID_COBORROWER
 )SELECT AS_ON_DATE,SOURCE,ENT_CIF,SRC_CIF,ACC_NO,ENT_CIF_COBORR
 FROM IBL_ENPA_STGDB.DBO.Prolendz_Stg_coborrower
...
WHERE [SP_Name]='Merg_Stg_Prolendz_Data' AND [EXT_DATE]=@EXT_DT AND ISNULL([Audit_Flg],0)=0
