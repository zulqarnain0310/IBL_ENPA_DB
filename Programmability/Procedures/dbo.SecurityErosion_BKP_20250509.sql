...
GO
CREATE PROCEDURE  [dbo].[SecurityErosion_BKP_20250509] 
(
@TimeKey int
)
AS
...
Select NCIF_Id into #NCIF from NPA_IntegrationDetails_20250509 With (nolock) where (EffectiveFromTimeKey=@TimeKey and EffectiveToTimeKey=@TimeKey)
...
INTO #Security
FROM NPA_IntegrationDetails_20250509 A With (nolock)
...
FROm #Erosion A
INNER JOIN NPA_IntegrationDetails_20250509 NID ON NID.EffectiveFromTimeKey<=@Timekey 
...
INNER JOIN NPA_IntegrationDetails_20250509 NIDC  ON NID.ImpactingAccountNo=NIDC.CustomerACID
