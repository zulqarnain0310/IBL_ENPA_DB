...
----(added on Syncing Activity 15-10-2024)
...
BEGIN
	...
	BEGIN
		...
		SET DATEFORMAT DMY
		 ...
		 DECLARE @Entity_Key Int   IF (@Entity_Key IS NULL)
				...
				DROP TABLE IF EXISTS #tmp
					Select distinct CustomerACID,EffectiveFromTimeKey,EffectiveToTimeKey,Accountentityid,STD_ASSET_CAT_Alt_key into #tmp ----STD_ASSET_CAT_Alt_key added on 20240925 by mohit
		...
		,UploadID
		,MOC_Remark
		,STD_ASSET_CAT_Alt_key  ----STD_ASSET_CAT_Alt_key added on 20240925 by mohit
		--(added on syncing activity 15-10-2024)
)
		...
		SELECT distinct
			...
			,N.STD_ASSET_CAT_Alt_key   ----STD_ASSET_CAT_Alt_key added on 20240925 by mohit
			---(added on syncing activity 15-10-2024)
		...
		LEFT JOIN DimMocReason  R ON R.MocReasonName = AccountLvlMOCDetails_stg.MOCReason 
									--(case when R.MocReasonName='Other reason of Account MOC' then 'Other' --COMMENTED ON PROD BY SHUBHAM KAMBLE DUE TO RECORD DUPLICACY ISSUE 14042024 
									--	  else R.MocReasonName END)= AccountLvlMOCDetails_stg.MOCReason 
								  and R.EffectiveToTimeKey=49999 AND R.MocReasonCategory='Account Reason'  --ADDED ON PROD BY SHUBHAM KAMBLE DUE TO RECORD DUPLICACY ISSUE 10042024 		
...
--------------------------------------------
	...
	BEGIN
		...
		SET 
			...
			,DateApprovedFirstLevel	= GETDATE()
		where UniqueUploadID=@UniqueUploadID
		AND UploadType='Account MOC Upload' ------ Added by Liyaqat 20240926
                ----(added on syncing activity 15-10-2024)
                ...
                (   
					...
					,EntityKey ----  Added on 20240719 by Lqt on UAT(added on syncing activity 15-10-2024)
				)
...
--								where A.EffectiveFromTimeKey=@TimeKey and A.EffectiveToTimeKey=@TimeKey
      ...
      ,FlgPNPA
															  ...
															  -- ,MOC_Reason
															  ,STD_ASSET_CAT_Alt_key  ----STD_ASSET_CAT_Alt_key added on 20240925 by mohit (added on syncing activity 15-10-2024)
												  )
					               	SELECT	DISTINCT
                                                              ...
                                                              ,A.ProvCoverGovGur
															   ...
															   ,A.IsFunded
															   ,B.UploadID -- Added by SSK on 14032023 to store MOC Upload ID
														--	   ,A.MOC_Reason
																,B.STD_ASSET_CAT_Alt_key ----STD_ASSET_CAT_Alt_key added on 20240925 by mohit(added on syncing activity 15-10-2024)
                                                   			from #ACCOUNT_CAL A
						...
						INSERT INTO PreMoc.NPA_IntegrationDetails
								(
					               ...
					               SELECT
														 ...
														 --  ,A.MOC_Reason
															,A.STD_ASSET_CAT_Alt_key ----STD_ASSET_CAT_Alt_key added on 20240925 by mohit(added on syncing activity 15-10-2024)
