...
--SELECT count(1) FROM npa_integrationdetails_PROD_6185_20230930 --31096332



-- EXEC [dbo].[ProvisionComputation] 26084
CREATE PROCEDURE [dbo].[ProvisionComputation_20231005_LIVE] (@TimeKey Smallint,@IS_MOC CHAR(1)='N')
WITH RECOMPILE
AS
...
FROM npa_integrationdetails_PROD_6185
...
from npa_integrationdetails_PROD_6185 A
...
ProvisionAlt_Key=(CASE WHEN IsFITL='Y' THEN @FITL
	...
	FROM npa_integrationdetails_PROD_6185 A --ADDED ON PROD 20230922
...
FROM npa_integrationdetails_PROD_6185 NID  
...
/*SCHEME_CODE IS  NULL */
UPDATE  NID SET NID.ProvisionAlt_Key = DPP.ProvisionAlt_key
		FROM npa_integrationdetails_PROD_6185 NID  INNER JOIN  
...
/* FOR UPDATING 6185 RECORD IN NPA_INTEGRATIONDETAILS TABLE 20231005*/

UPDATE A SET A.AddlProvision=B.AddlProvision
			,A.TotalProvision=B.TotalProvision
			,A.ProvisionAlt_Key=B.ProvisionAlt_Key
FROM npa_integrationdetails A INNER JOIN npa_integrationdetails_PROD_6185 B ON A.customeracid = B.CUSTOMERACID
and A.EffectiveFromTimeKey=26936--COUNT 
...
UPDATE npa_integrationdetails_PROD_6185 SET FlgProcessing='N'
WHERE EffectiveFromTimeKey<=@TimeKey
AND EffectiveToTimeKey>=@TimeKey
AND ISNULL(FlgProcessing,'N')='Y'
