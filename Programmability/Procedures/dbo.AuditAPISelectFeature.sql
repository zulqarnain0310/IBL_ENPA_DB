...
CREATE PROCEDURE [dbo].[AuditAPISelectFeature]  
       @PANNo            VARCHAR(10) = N'',
...
        
		   ...
		   If(@PANNo_final<>'xxxx' or @VoterId_final<>'xxxx' or @Aadhar_final<>'XXXX' or @NCIF_Id_final<>'XXXX')
			...
			Drop table if exists #ucic
				...
				CREATE INDEX Idx_UCIC on #ucic(UCIC)
...
            LEFT JOIN CURDAT.AdvAcRestructureDetail_Curnt (nolock) C
		...
		If (@NCIF_Id_final<>'xxxx') and (@CountPAN=0)
				begin   
					...
					DROP  TABLE  IF exists #CoBo_Check
					   ...
					   into #CoBo_Check from  dbo.CoBorrowerData_curnt (nolock) where NCIFID_COBORROWER in (@NCIF_Id_final) OR NCIFID_COBORROWER in  (select UCIC from #ucic)
				 
					IF EXISTS (select 1 from #CoBo_Check)
						 ...
						 Begin
							...
							--		Inner Join dbo.NPA_IntegrationDetails_Curnt (nolock) c on a.NCIFID_PrimaryAccount=c.NCIF_Id --AND  (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
							--		Left Join dbo.NPA_IntegrationDetails_Curnt (nolock) b  on a.NCIFID_COBORROWER=b.NCIF_Id  --AND (B.EffectiveFromTimeKey<=@TimeKey AND B.EffectiveToTimeKey>=@TimeKey)
							--		 where (a.NCIFID_COBORROWER=@NCIF_Id_final OR a.NCIFID_COBORROWER in (select UCIC from #ucic))
						...
						Drop table if exists #NCIF_NIdata
								...
								Drop table if exists #Cobodata
								select distinct a.NCIFID_COBORROWER,a.AcDegDate,a.AcDegFlg
								into #Cobodata from dbo.CoBorrowerData_curnt (nolock) a 
									Inner Join #NCIF_NIdata (nolock) c on a.NCIFID_PrimaryAccount=c.NCIF_Id --AND  (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
									Left Join #NCIF_NIdata (nolock) b  on a.NCIFID_COBORROWER=b.NCIF_Id  --AND (B.EffectiveFromTimeKey<=@TimeKey AND B.EffectiveToTimeKey>=@TimeKey)
									 where (a.NCIFID_COBORROWER=@NCIF_Id_final OR a.NCIFID_COBORROWER in (select UCIC from #ucic))
									 and b.NCIF_Id is null --and a.AcDegFlg='Y'
