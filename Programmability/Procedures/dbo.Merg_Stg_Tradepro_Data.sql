...
CREATE PROC [dbo].[Merg_Stg_Tradepro_Data]
WITH RECOMPILE
AS
...
DECLARE @SrcSysAlt_Key Smallint=(SELECT SourceAlt_Key FROM [dbo].[DimSourceSystem] WHERE SourceName='Tradepro' AND  EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
...
DECLARE @Daily_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Tradepro_Stg_Daily_Error)
DECLARE @Incremental_COUNT INT=(SELECT COUNT(1) FROM IBL_ENPA_STGDB.dbo.Tradepro_Stg_Incremental_Error)
...
SELECT @DataProcessingFlg=(CASE WHEN @Incremental_COUNT>0 OR @Daily_COUNT>0 
                                   OR  (SELECT ISNULL(COUNT(1),0) FROM IBL_ENPA_STGDB.DBO.Tradepro_Stg_Daily WHERE AsOnDate=@Exec_Date)=0
                                    THEN 0
                               ELSE 1
                           END)  
...
INSERT INTO [dbo].[LagDateDataDetails]
           ...
           ,[SourceSystem]
           ,[DataDateUsed])
SELECT @Exec_Date,'Tradepro',@DATEDATAUSE
END 

--Audit 
DELETE IBL_ENPA_STGDB.[dbo].[Procedure_Audit] 
WHERE [SP_Name]='Merg_Stg_Tradepro_Data' AND [EXT_DATE]=GETDATE() AND ISNULL([Audit_Flg],0)=0

INSERT INTO IBL_ENPA_STGDB.[dbo].[Procedure_Audit]
           ([EXT_DATE] ,[Timekey] ,[SP_Name],Start_Date_Time )
SELECT @Exec_Date,@TimeKey,'Merg_Stg_Tradepro_Data',GETDATE()
...
INNER JOIN IBL_ENPA_STGDB.dbo.Tradepro_Stg_Daily PSD ON --NPA.SrcSysAlt_Key=@SrcSysAlt_Key
...
SET 
    ...
    Retail_Corpo=FI.Retail_Corpo  -----  Added on 17092022 for identifying force asset class update
...
INNER JOIN IBL_ENPA_STGDB.dbo.Tradepro_Stg_Incremental  FI ON EffectiveFromTimeKey<=@TimeKey
...
WHERE [SP_Name]='Merg_Stg_Tradepro_Data' AND [EXT_DATE]=@EXT_DT AND ISNULL([Audit_Flg],0)=0
