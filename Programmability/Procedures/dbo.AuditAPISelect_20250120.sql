...
Create PROCEDURE [dbo].[AuditAPISelect_20250120]  
      @PANNo            VARCHAR(10) = N'',  
...
      begin  
--      DECLARE @CountPack INT=(SELECT COUNT(*) FROM IBL_ENPA_STGDB.dbo.Package_Audit(nolock) 
--										WHERE CAST(ExecutionStartTime AS DATE)=CAST(GETDATE() AS DATE))  
...
            begin  
					...
					IF len(isnull(@NCIF_Id,''))>0 AND  Exists (select 1 from  CoBorrowerData_curnt where NCIFID_COBORROWER=@NCIF_Id OR NCIFID_COBORROWER in (select UCIC from #ucicdata2))
							Begin 

								Drop Table If Exists #Cobodata
									select distinct a.*
									into #Cobodata from CoBorrowerData_curnt a 
										Inner Join NPA_IntegrationDetails_Curnt c on a.NCIFID_PrimaryAccount=c.NCIF_Id
										Left Join NPA_IntegrationDetails_Curnt b 
