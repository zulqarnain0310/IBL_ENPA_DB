...
CREATE Proc [dbo].[PROC_HistoryDownGradationData]
as
Begin
 ...
 Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 1,'HistoryDownGradation','NPA Curnt Table Creation',GETDATE(),@Ext_Date,'Y'
 ...
 Drop Table if exists #npa_integrationdetails_Curnt
  ...
  Update SPAuditCheck set endtime=GETDATE() where ID=1 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date

    Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 2,'HistoryDownGradation','NPA archive Table Creation',GETDATE(),@Ext_Date,'Y'
   Drop Table if exists #npa_integrationdetails_archive
  Select * into #npa_integrationdetails_archive
  from [dbo].[NPA_IntegrationDetails_Archive]
  where EffectiveFromTimeKey=(Select @Timekey-1) and EffectiveToTimeKey=(Select @Timekey-1)   --- 14 dec 2022

   Update SPAuditCheck set endtime=GETDATE() where ID=2 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date
  
      Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 3,'HistoryDownGradation','index creation NPA table',GETDATE(),@Ext_Date,'Y'
  Create  NOnclustered index idx_npa_integrationdetails_archive on #npa_integrationdetails_archive(customeracid)
  Update SPAuditCheck set endtime=GETDATE() where ID=3 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date

Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 4,'HistoryDownGradation','Insertion into HistoryDownGradeData from temp table',GETDATE(),@Ext_Date,'Y'
...
from #npa_integrationdetails_Curnt NPD 
 ...
 and NPD.NCIF_AssetClassAlt_Key<>1 and NPDArc.NCIF_AssetClassAlt_Key=1 	
  ...
  Update SPAuditCheck set endtime=GETDATE() where ID=4 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date

   Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 5,'HistoryDownGradation','additional account creation',GETDATE(),@Ext_Date,'Y'
 ...
 Update SPAuditCheck set endtime=GETDATE() where ID=5 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date

   Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 6,'HistoryDownGradation','insertion into historydowngrade for additional column',GETDATE(),@Ext_Date,'Y'
 Insert into HistoryDownGradeData
  (
  AsonDate
,[SourceName]
...
,NPD.flgdeg
 ...
 Update SPAuditCheck set endtime=GETDATE() where ID=6 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date
 
    Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 7,'HistoryDownGradation','single account culprit marking',GETDATE(),@Ext_Date,'Y'
  ...
  group by NCIF
  having count(1)=1)
   Update SPAuditCheck set endtime=GETDATE() where ID=7 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date
  
      Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 8,'HistoryDownGradation','multiple account culprit marking',GETDATE(),@Ext_Date,'Y'
  ...
  having count(1)>1) group by NCIF) b on a.NCIF=b.NCIF
   ...
   Update SPAuditCheck set endtime=GETDATE() where ID=8 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date

         Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 9,'HistoryDownGradation','multiple account culprit marking with source1',GETDATE(),@Ext_Date,'Y'
  ...
  */
  Drop table if exists #HistoryDownGradeData
  Select * into #HistoryDownGradeData from HistoryDownGradeData where AsonDate=@Ext_Date

   Update a set [Culprit System]=b.SourceName,[Culprit System Account No]=b.Account from
  (select CIF,NCIF,Account,[Date of NPA of source system],[Homogenized_Date of NPA],
  [Classification of source system],[Homogenized_Classification],[Culprit System],
  [Culprit System Account No]
  from HistoryDownGradeData(nolock) A where 
  (isnull([Classification of source system],1)=isnull([Homogenized_Classification],1) or
    isnull([Date of NPA of source system],'')=isnull([Homogenized_Date of NPA],''))
  and AsonDate=@Ext_Date and NCIF in (
  Select NCIF from  #HistoryDownGradeData  
  group by NCIF
  having count(1)>1)) a 
  inner join
   (select NCIF, STRING_AGG (CONVERT(NVARCHAR(max),Sourcename),',') Sourcename,STRING_AGG (CONVERT(NVARCHAR(max),Account),',') Account
  from #HistoryDownGradeData(nolock) A where 
   ( isnull([Classification of source system],1)=isnull([Homogenized_Classification],1) or
    isnull([Date of NPA of source system],'')=isnull([Homogenized_Date of NPA],''))
    and  NCIF in (
  Select NCIF from  #HistoryDownGradeData(nolock)  
  group by NCIF
  having count(1)>1) group by NCIF) b on a.NCIF=b.NCIF
   
  Update SPAuditCheck set endtime=GETDATE() where ID=9 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date
    
	         Insert into SPAuditCheck (ID,SPName,StepName,StartTime,Ext_Date,ISsucess)
 select 10,'HistoryDownGradation','multiple account culprit marking with source2',GETDATE(),@Ext_Date,'Y'
  ...
  Update SPAuditCheck set endtime=GETDATE() where ID=10 and SPName='HistoryDownGradation' and Ext_Date=@Ext_Date
  /*
;with cte as
(
select   
