...
CREATE PROCEDURE  [dbo].[ProvisionPolicyStageDataInUp] 
	@Timekey INT,
...
print '0'			
		...
		/*FOR NEW SCHEME CODE ADDITION*/
							...
							--	,ProvisionAlt_key
								...
								,CAST(P.[3monthsupto6months] as decimal(18,2))/100 as From_4_months_upto_6_months
								,CAST(P.[6monthsupto9months] as decimal(18,2))/100 as From_7_months_upto_9_months
								,CAST(P.[9monthsupto12months] as decimal(18,2))/100 as From_10_months_upto_12_months
	...
	/*TO UPDATE THE PERCENTAGES OF EXITSTING SCHEME CODE FROM MAIN TABLE--SHUBHAM KAMBLE*/
			SET IDENTITY_INSERT DIMPROVISIONPOLICY_mod ON;
						INSERT INTO  DIMPROVISIONPOLICY_mod (
										...
										,Case When ISNULL(P.[3monthsupto6months],'')='' Then CAST(DP.From_4_months_upto_6_months AS DECIMAL(18,2)) Else CAST(P.[3monthsupto6months] as decimal(18,2))/100  END From_4_months_upto_6_months
										,Case When ISNULL(P.[6monthsupto9months],'')='' Then CAST(DP.From_7_months_upto_9_months AS DECIMAL(18,2)) Else CAST(P.[6monthsupto9months] as decimal(18,2))/100  END From_7_months_upto_9_months
										,Case When ISNULL(P.[9monthsupto12months],'')='' Then CAST(DP.From_10_months_upto_12_months AS DECIMAL(18,2)) Else CAST(P.[9monthsupto12months] as decimal(18,2))/100 END From_10_months_upto_12_months
...
print '2'
	/*TO UPDATE THE PERCENTAGES OF EXITSTING SCHEME CODE WHICH ARE NULL FROM MAIN TABLE --SHUBHAM KAMBLE*/										
						INSERT INTO  DIMPROVISIONPOLICY_mod (
										...
										,DP.Scheme_Code
										,Case When ISNULL(P.upto3months,'')='' Then CAST(DP.upto_3_months AS DECIMAL(18,2)) Else CAST(P.upto3months as decimal(18,2))/100  END upto_3_months
										,Case When ISNULL([3monthsupto6months],'')='' Then CAST(DP.From_4_months_upto_6_months AS DECIMAL(18,2)) Else CAST([3monthsupto6months] as decimal(18,2))/100  END From_4_months_upto_6_months
										,Case When ISNULL(P.[6monthsupto9months],'')='' Then CAST(DP.From_7_months_upto_9_months AS DECIMAL(18,2)) Else CAST(P.[6monthsupto9months] as decimal(18,2))/100  END From_7_months_upto_9_months
										,Case When ISNULL(P.[9monthsupto12months],'')='' Then CAST(DP.From_10_months_upto_12_months AS DECIMAL(18,2)) Else CAST(P.[9monthsupto12months] as decimal(18,2))/100 END From_10_months_upto_12_months
										,Case When ISNULL(P.Doubtful1,'')='' Then CAST(DP.Doubtful_1 AS DECIMAL(18,2)) Else CAST(P.Doubtful1 as decimal(18,2))/100  END Doubtful_1
										,Case When ISNULL(P.Doubtful2,'')='' Then CAST(DP.Doubtful_2 AS DECIMAL(18,2)) Else CAST(P.Doubtful2 as decimal(18,2))/100 END Doubtful_2
...
IF (@OperationFlag=20)
	...
	BEGIN
		...
		UPDATE A
					...
					FROM DIMPROVISIONPOLICY A
					INNER JOIN DIMPROVISIONPOLICY_MOD B ON A.ProvisionAlt_key=B.ProvisionAlt_key AND A.Scheme_Code=B.Scheme_Code
					WHERE (A.EffectiveFromTimeKey<=@Timekey AND A.EffectiveToTimeKey >=@Timekey)
					AND B.EffectiveFromTimeKey<=@Timekey AND B.EffectiveToTimeKey >=@Timekey
					AND B.UploadID=@UniqueUploadID
					AND B.AuthorisationStatus IN ('D')
		
		/*EXPIRING OLD RECORD FROM MOD TABLE*/  --20241211 MOHIT
		UPDATE A
						SET A.EffectiveToTimeKey =@Timekey -1
						,A.ModifiedBy=@UserLoginID
						,A.DateModified=GETDATE()
					...
					INNER JOIN DIMPROVISIONPOLICY B ON A.ProvisionAlt_key=B.ProvisionAlt_key AND A.Scheme_Code=B.Scheme_Code
					WHERE (A.EffectiveFromTimeKey<=@Timekey AND A.EffectiveToTimeKey >=@Timekey)
					AND B.EffectiveFromTimeKey<=@Timekey AND B.EffectiveToTimeKey >=@Timekey
					AND A.UploadID<>B.UploadID
					AND B.AuthorisationStatus IN ('D')--MOHIT
	...
	--			AND Scheme_Code IN (SELECT Scheme_Code FROM DIMPROVISIONPOLICY WHERE Scheme_Code IS NOT NULL)
		...
		WHERE A.EffectiveFromTimeKey<=@Timekey AND A.EffectiveToTimeKey>=@Timekey
		AND B.EffectiveFromTimeKey<=@Timekey AND B.EffectiveToTimeKey>=@Timekey
		AND A.AuthorisationStatus IN ('D') AND B.AuthorisationStatus IN ('A')
